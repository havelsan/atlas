...
#region Methods
        ...
        public override void TaskScript()
        {
            ...
            if (string.IsNullOrEmpty(FilePath) == true)
                ...
                filename = System.IO.Path.Combine(FilePath, "MORRECETEILACLARI.txt");

            try
            {
                PurplePrescriptionDrugList.DrugListWebCaller callerObject = new PurplePrescriptionDrugList.DrugListWebCaller();
                ...
                YardimciIslemler.ilacListesiSorguCevapDVO responsePurplePrescriptionDrugList = YardimciIslemler.WebMethods.morReceteIlacListesiSorgulaSync(Sites.SiteLocalHost, request);
                if (responsePurplePrescriptionDrugList.sonucKodu == "0000")
                {
                    if (responsePurplePrescriptionDrugList.ilacListesi != null)
                    {
                        string s = "";
                        foreach (YardimciIslemler.ilacDVO item in responsePurplePrescriptionDrugList.ilacListesi)
                        {
                            count++;
                            if (count == 1)
                            {
                        ...
                        AddLog(TTUtils.CultureService.GetText("M26537", "Mor Reçete İlaçları aktarıldı."));
                    }
                }
                else
                    throw new Exception(responsePurplePrescriptionDrugList.sonucMesaji);
