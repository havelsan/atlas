...
#region Methods
        ...
        public override void TaskScript()
        {
            ...
            if (string.IsNullOrEmpty(FilePath) == true)
                ...
                filename = System.IO.Path.Combine(FilePath, "TURUNCURECETEILACLARI.txt");

            try
            {
                OrangePrescriptionDrugList.DrugListWebCaller callerObject = new OrangePrescriptionDrugList.DrugListWebCaller();
                ...
                YardimciIslemler.ilacListesiSorguCevapDVO responseOrangePrescriptionDrugList = YardimciIslemler.WebMethods.turuncuReceteIlacListesiSorgulaSync(Sites.SiteLocalHost, request);
                if (responseOrangePrescriptionDrugList.sonucKodu == "0000")
                {
                    if (responseOrangePrescriptionDrugList.ilacListesi != null)
                    {
                        string s = "";
                        foreach (YardimciIslemler.ilacDVO item in responseOrangePrescriptionDrugList.ilacListesi)
                        {
                            count++;
                            if (count == 1)
                            {
                        ...
                        AddLog("Turuncu Reçete İlaçları aktarıldı.");
                    }
                }
                else
                    throw new Exception(responseOrangePrescriptionDrugList.sonucMesaji);
