...
#region Methods
        ...
        public override void TaskScript()
        {
            ...
            if (string.IsNullOrEmpty(FilePath) == true)
                ...
                filename = System.IO.Path.Combine(FilePath, "NORMALRECETEILACLARI.txt");

            try
            {
                NormalPrescriptionDrugList.DrugListWebCaller callerObject = new NormalPrescriptionDrugList.DrugListWebCaller();
                ...
                YardimciIslemler.ilacListesiSorguCevapDVO responseNormalPrescriptionDrugList = YardimciIslemler.WebMethods.normalReceteIlacListesiSorgulaSync(Sites.SiteLocalHost, request);
                if (responseNormalPrescriptionDrugList.sonucKodu == "0000")
                {
                    if (responseNormalPrescriptionDrugList.ilacListesi != null)
                    {
                        string s = "";
                        foreach (YardimciIslemler.ilacDVO item in responseNormalPrescriptionDrugList.ilacListesi)
                        {
                            count++;
                            if (count == 1)
                            {
                        ...
                        AddLog(TTUtils.CultureService.GetText("M26587", "Normal Reçete İlaçları aktarıldı."));
                    }
                }
                else
                    throw new Exception(responseNormalPrescriptionDrugList.sonucMesaji);
