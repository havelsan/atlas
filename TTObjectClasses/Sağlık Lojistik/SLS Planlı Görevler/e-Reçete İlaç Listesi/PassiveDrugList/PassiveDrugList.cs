...
#region Methods
         ...
         public override void TaskScript()
        {
            ...
            if (string.IsNullOrEmpty(FilePath) == true)
                ...
                filename = System.IO.Path.Combine(FilePath, "GERIODEMEOLMAYANILAC.txt");

            try
            {
                PassiveDrugList.DrugListWebCaller callerObject = new PassiveDrugList.DrugListWebCaller();
                ...
                YardimciIslemler.ilacListesiSorguCevapDVO responsePassiveDrugList = YardimciIslemler.WebMethods.pasifIlacListesiSorgulaSync(Sites.SiteLocalHost, request);
                if (responsePassiveDrugList.sonucKodu == "0000")
                {
                    if (responsePassiveDrugList.ilacListesi != null)
                    {
                        string s = "";
                        foreach (YardimciIslemler.ilacDVO item in responsePassiveDrugList.ilacListesi)
                        {
                            count++;
                            if (count == 1)
                            {
                        ...
                        AddLog(TTUtils.CultureService.GetText("M25706", "Geri ödeme kapsamında olmayan ilaçları aktarıldı."));
                    }
                }
                else
                    throw new Exception(responsePassiveDrugList.sonucMesaji);
