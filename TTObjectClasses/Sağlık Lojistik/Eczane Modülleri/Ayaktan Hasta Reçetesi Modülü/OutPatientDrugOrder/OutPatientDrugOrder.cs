...
#region PreTransition_New2Completed
        ...
        {
            if (transDef.ObjectDef.CodeName != typeof(OutPatientDrugOrder).Name)
                return;

            var fromState = transDef.FromStateDefID;
            var toState = transDef.ToStateDefID;

            if (fromState == OutPatientDrugOrder.States.New && toState == OutPatientDrugOrder.States.Completed)
