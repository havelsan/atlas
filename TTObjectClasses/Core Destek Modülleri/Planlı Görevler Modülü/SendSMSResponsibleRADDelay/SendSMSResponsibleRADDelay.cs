        ...
        public override void TaskScript()
        {
            ...
            if (smsRadiologyActive == "TRUE")
            {
                ...
                if (dayLimit != "")
                {
                    ...
                    BindingList<SubactionProcedureSMSInfo> smsList = SubactionProcedureSMSInfo.GetSPSendToBeSMSInfo(objectContext, "RADIOLOGYTEST", Convert.ToInt32(dayLimit) + 1, " AND SMSNUMBERFORRESPONSIBLE = 0"); //Sms gönderilecek radyolojiler

                    foreach (SubactionProcedureSMSInfo smsInfo in smsList)
                    {
                        try
                        {
                            var sysParameter = TTObjectClasses.SystemParameter.GetParameterValue("RESPONSIBLEIDFORRADIOLOGYSMS", "");
                            ...
                            if (user.PhoneNumber != "")
                            {
                                ...
                                string day = (Convert.ToInt32(dayLimit) + 1).ToString();
                                ...
                                userMessage.SendSMSPerson(users, messageText, SMSTypeEnum.RadiologyReportDelaySMSForResponsible);
                                smsInfo.SMSNumberForResponsible++;
                                smsInfo.ResponsibleUserID = user.ObjectID;
                                objectContext.Save();
                            }
                        }
                        catch (Exception)
