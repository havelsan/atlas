...
#region Methods
        public override List<RuleBase.RuleResult> RunRule(DateTime actionDate, ISUTInstance currentInstance)
        {
            ...
            {
                throw new TTException(SystemMessage.GetMessageV3(544,new string[] {currentInstance.GetSUTRulableObject().ToString()}));
            }
            return retvalue;
        }
        ...
        private void GetSuitableDates(DateTime actionDate, out DateTime startDate, out DateTime endDate)
        {
            if (DayCount.Value > 1)
            {
                DateTime d = actionDate.AddDays(-1 * (DayCount.Value - 1));
            ...
            {
                startDate = new DateTime(actionDate.Year, actionDate.Month, actionDate.Day, 0, 0, 0);
                endDate = new DateTime(actionDate.Year, actionDate.Month, actionDate.Day, 23, 59, 59);
            }
        }
