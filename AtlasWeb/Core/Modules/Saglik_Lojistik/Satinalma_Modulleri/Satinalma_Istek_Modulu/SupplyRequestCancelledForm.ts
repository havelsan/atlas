...
import { SupplyRequestCancelledFormViewModel } from './SupplyRequestCancelledFormViewModel';
...
@Component({
    selector: 'SupplyRequestCancelledForm',
    templateUrl: './SupplyRequestCancelledForm.html',
    providers: [MessageService]
})
export class SupplyRequestCancelledForm extends BaseSupplyRequest implements OnInit {
    public SupplyRequestDetailsColumns = [];
    public supplyRequestCancelledFormViewModel: SupplyRequestCancelledFormViewModel = new SupplyRequestCancelledFormViewModel();
    public get _SupplyRequest(): SupplyRequest {
        return this._TTObject as SupplyRequest;
    }
    private SupplyRequestCancelledForm_DocumentUrl: string = '/api/SupplyRequestService/SupplyRequestCancelledForm';
    constructor(protected httpService: NeHttpService,
        protected messageService: MessageService,
        protected ngZone: NgZone) {
        super(httpService, messageService, ngZone);
        this._DocumentServiceUrl = this.SupplyRequestCancelledForm_DocumentUrl;
        this.initViewModel();
    ...
    public initViewModel(): void {
        ...
        this.supplyRequestCancelledFormViewModel = new SupplyRequestCancelledFormViewModel();
        this._ViewModel = this.supplyRequestCancelledFormViewModel;
        this.supplyRequestCancelledFormViewModel._SupplyRequest = this._TTObject as SupplyRequest;
        this.supplyRequestCancelledFormViewModel._SupplyRequest.SignUser = new ResUser();
        this.supplyRequestCancelledFormViewModel._SupplyRequest.DestinationStore = new Store();
        this.supplyRequestCancelledFormViewModel._SupplyRequest.SupplyRequestDetails = new Array<SupplyRequestDetail>();
        this.supplyRequestCancelledFormViewModel._SupplyRequest.Store = new Store();
    }

    protected loadViewModel() {
        let that = this;

        that.supplyRequestCancelledFormViewModel = this._ViewModel as SupplyRequestCancelledFormViewModel;
        that._TTObject = this.supplyRequestCancelledFormViewModel._SupplyRequest;
        if (this.supplyRequestCancelledFormViewModel == null)
            this.supplyRequestCancelledFormViewModel = new SupplyRequestCancelledFormViewModel();
        if (this.supplyRequestCancelledFormViewModel._SupplyRequest == null)
            this.supplyRequestCancelledFormViewModel._SupplyRequest = new SupplyRequest();
        let signUserObjectID = that.supplyRequestCancelledFormViewModel._SupplyRequest["SignUser"];
        if (signUserObjectID != null && (typeof signUserObjectID === 'string')) {
            let signUser = that.supplyRequestCancelledFormViewModel.ResUsers.find(o => o.ObjectID.toString() === signUserObjectID.toString());
             if (signUser) {
                that.supplyRequestCancelledFormViewModel._SupplyRequest.SignUser = signUser;
            }
        }
        let destinationStoreObjectID = that.supplyRequestCancelledFormViewModel._SupplyRequest["DestinationStore"];
        if (destinationStoreObjectID != null && (typeof destinationStoreObjectID === 'string')) {
            let destinationStore = that.supplyRequestCancelledFormViewModel.Stores.find(o => o.ObjectID.toString() === destinationStoreObjectID.toString());
             if (destinationStore) {
                that.supplyRequestCancelledFormViewModel._SupplyRequest.DestinationStore = destinationStore;
            }
        }
        that.supplyRequestCancelledFormViewModel._SupplyRequest.SupplyRequestDetails = that.supplyRequestCancelledFormViewModel.SupplyRequestDetailsGridList;
        for (let detailItem of that.supplyRequestCancelledFormViewModel.SupplyRequestDetailsGridList) {
            let materialObjectID = detailItem["Material"];
            if (materialObjectID != null && (typeof materialObjectID === 'string')) {
                let material = that.supplyRequestCancelledFormViewModel.Materials.find(o => o.ObjectID.toString() === materialObjectID.toString());
                 if (material) {
            ...
            if (purchaseGroupObjectID != null && (typeof purchaseGroupObjectID === 'string')) {
                let purchaseGroup = that.supplyRequestCancelledFormViewModel.PurchaseGroups.find(o => o.ObjectID.toString() === purchaseGroupObjectID.toString());
                 if (purchaseGroup) {
            ...
            if (distributionTypeObjectID != null && (typeof distributionTypeObjectID === 'string')) {
                let distributionType = that.supplyRequestCancelledFormViewModel.DistributionTypeDefinitions.find(o => o.ObjectID.toString() === distributionTypeObjectID.toString());
                 if (distributionType) {
                    detailItem.DistributionType = distributionType;
                }
            }
        }
        let storeObjectID = that.supplyRequestCancelledFormViewModel._SupplyRequest["Store"];
        if (storeObjectID != null && (typeof storeObjectID === 'string')) {
            let store = that.supplyRequestCancelledFormViewModel.Stores.find(o => o.ObjectID.toString() === storeObjectID.toString());
             if (store) {
                that.supplyRequestCancelledFormViewModel._SupplyRequest.Store = store;
            }
        }
    }
    ...
    async ngOnInit() {
        let that = this;
        await this.load(SupplyRequestCancelledFormViewModel);
        this.FormCaption = i18n("M21367", "Satınalma Talep ( İptal Edildi )");
    ...
    public initFormControls(): void {
        ...
        this.AutoMaterialButton = new TTVisual.TTButtonColumn();
        this.AutoMaterialButton.Text = "Malzeme Getir";
