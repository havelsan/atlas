...
import { SupplyRequestCompletedFormViewModel } from './SupplyRequestCompletedFormViewModel';
...
@Component({
    selector: 'SupplyRequestCompletedForm',
    templateUrl: './SupplyRequestCompletedForm.html',
    providers: [MessageService]
})
export class SupplyRequestCompletedForm extends BaseSupplyRequest implements OnInit {
    public SupplyRequestDetailsColumns = [];
    public supplyRequestCompletedFormViewModel: SupplyRequestCompletedFormViewModel = new SupplyRequestCompletedFormViewModel();
    public get _SupplyRequest(): SupplyRequest {
        return this._TTObject as SupplyRequest;
    }
    private SupplyRequestCompletedForm_DocumentUrl: string = '/api/SupplyRequestService/SupplyRequestCompletedForm';
    constructor(protected httpService: NeHttpService,
                protected messageService: MessageService,
                protected ngZone: NgZone) {
        super(httpService, messageService, ngZone);
        this._DocumentServiceUrl = this.SupplyRequestCompletedForm_DocumentUrl;
        this.initViewModel();
    ...
    public initViewModel(): void {
        ...
        this.supplyRequestCompletedFormViewModel = new SupplyRequestCompletedFormViewModel();
        this._ViewModel = this.supplyRequestCompletedFormViewModel;
        this.supplyRequestCompletedFormViewModel._SupplyRequest = this._TTObject as SupplyRequest;
        this.supplyRequestCompletedFormViewModel._SupplyRequest.SignUser = new ResUser();
        this.supplyRequestCompletedFormViewModel._SupplyRequest.DestinationStore = new Store();
        this.supplyRequestCompletedFormViewModel._SupplyRequest.SupplyRequestDetails = new Array<SupplyRequestDetail>();
        this.supplyRequestCompletedFormViewModel._SupplyRequest.Store = new Store();
    }

    protected loadViewModel() {
        let that = this;

        that.supplyRequestCompletedFormViewModel = this._ViewModel as SupplyRequestCompletedFormViewModel;
        that._TTObject = this.supplyRequestCompletedFormViewModel._SupplyRequest;
        if (this.supplyRequestCompletedFormViewModel == null)
            this.supplyRequestCompletedFormViewModel = new SupplyRequestCompletedFormViewModel();
        if (this.supplyRequestCompletedFormViewModel._SupplyRequest == null)
            this.supplyRequestCompletedFormViewModel._SupplyRequest = new SupplyRequest();
        let signUserObjectID = that.supplyRequestCompletedFormViewModel._SupplyRequest["SignUser"];
        if (signUserObjectID != null && (typeof signUserObjectID === 'string')) {
            let signUser = that.supplyRequestCompletedFormViewModel.ResUsers.find(o => o.ObjectID.toString() === signUserObjectID.toString());
             if (signUser) {
                that.supplyRequestCompletedFormViewModel._SupplyRequest.SignUser = signUser;
            }
        }
        let destinationStoreObjectID = that.supplyRequestCompletedFormViewModel._SupplyRequest["DestinationStore"];
        if (destinationStoreObjectID != null && (typeof destinationStoreObjectID === 'string')) {
            let destinationStore = that.supplyRequestCompletedFormViewModel.Stores.find(o => o.ObjectID.toString() === destinationStoreObjectID.toString());
             if (destinationStore) {
                that.supplyRequestCompletedFormViewModel._SupplyRequest.DestinationStore = destinationStore;
            }
        }
        that.supplyRequestCompletedFormViewModel._SupplyRequest.SupplyRequestDetails = that.supplyRequestCompletedFormViewModel.SupplyRequestDetailsGridList;
        for (let detailItem of that.supplyRequestCompletedFormViewModel.SupplyRequestDetailsGridList) {
            let materialObjectID = detailItem["Material"];
            if (materialObjectID != null && (typeof materialObjectID === 'string')) {
                let material = that.supplyRequestCompletedFormViewModel.Materials.find(o => o.ObjectID.toString() === materialObjectID.toString());
                 if (material) {
            ...
            if (purchaseGroupObjectID != null && (typeof purchaseGroupObjectID === 'string')) {
                let purchaseGroup = that.supplyRequestCompletedFormViewModel.PurchaseGroups.find(o => o.ObjectID.toString() === purchaseGroupObjectID.toString());
                 if (purchaseGroup) {
            ...
            if (distributionTypeObjectID != null && (typeof distributionTypeObjectID === 'string')) {
                let distributionType = that.supplyRequestCompletedFormViewModel.DistributionTypeDefinitions.find(o => o.ObjectID.toString() === distributionTypeObjectID.toString());
                 if (distributionType) {
                    detailItem.DistributionType = distributionType;
                }
            }
        }
        let storeObjectID = that.supplyRequestCompletedFormViewModel._SupplyRequest["Store"];
        if (storeObjectID != null && (typeof storeObjectID === 'string')) {
            let store = that.supplyRequestCompletedFormViewModel.Stores.find(o => o.ObjectID.toString() === storeObjectID.toString());
             if (store) {
                that.supplyRequestCompletedFormViewModel._SupplyRequest.Store = store;
            }
        }
    }
    ...
    async ngOnInit()  {
        let that = this;
        await this.load(SupplyRequestCompletedFormViewModel);
        this.FormCaption = i18n("M21368", "Satınalma Talep ( Tamamlandı )");
