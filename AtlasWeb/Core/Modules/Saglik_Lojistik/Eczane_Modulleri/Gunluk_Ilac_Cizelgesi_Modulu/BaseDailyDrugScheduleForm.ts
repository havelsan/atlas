...
import { BaseDailyDrugScheduleFormViewModel } from './BaseDailyDrugScheduleFormViewModel';
...
@Component({
    selector: 'BaseDailyDrugScheduleForm',
    templateUrl: './BaseDailyDrugScheduleForm.html',
    providers: [MessageService]
})
export class BaseDailyDrugScheduleForm extends StockActionBaseForm implements OnInit {
    ActionDate: TTVisual.ITTDateTimePicker;
    DailyDrugOrderGetByDate: TTVisual.ITTButton;
    DailyDrugPatientsGrid: TTVisual.ITTGrid;
    DailyDrugSchOrdersGrid: TTVisual.ITTGrid;
    DailyDrugUnDrugsGrid: TTVisual.ITTGrid;
    DestinationStore: TTVisual.ITTObjectListBox;
    DoseAmountDailyDrugSchOrder: TTVisual.ITTTextBoxColumn;
    DoseAmountDailyDrugUnDrug: TTVisual.ITTTextBoxColumn;
    DrugListTab: TTVisual.ITTTabPage;
    drugOrderName: TTVisual.ITTTextBoxColumn;
    ...
    patientNameSurname: TTVisual.ITTTextBoxColumn;
    PatientTab: TTVisual.ITTTabPage;
    patinetFullName: TTVisual.ITTTextBoxColumn;
    selecetedPatient: TTVisual.ITTCheckBoxColumn;
    ...
    UnDrugListTab: TTVisual.ITTTabPage;
    UnDrugName: TTVisual.ITTTextBoxColumn;
    public DailyDrugPatientsGridColumns = [];
    public DailyDrugSchOrdersGridColumns = [];
    public DailyDrugUnDrugsGridColumns = [];
    public baseDailyDrugScheduleFormViewModel: BaseDailyDrugScheduleFormViewModel = new BaseDailyDrugScheduleFormViewModel();
    public get _DailyDrugSchedule(): DailyDrugSchedule {
        return this._TTObject as DailyDrugSchedule;
    }
    private BaseDailyDrugScheduleForm_DocumentUrl: string = '/api/DailyDrugScheduleService/BaseDailyDrugScheduleForm';
    constructor(protected httpService: NeHttpService,
                protected messageService: MessageService,
                protected ngZone: NgZone) {
        super(httpService, messageService, ngZone);
        this._DocumentServiceUrl = this.BaseDailyDrugScheduleForm_DocumentUrl;
        this.initViewModel();
    ...
    protected async PreScript() {
        ...
        if (this._DailyDrugSchedule.Store == null) {
            this._DailyDrugSchedule.Store = this.inputStore;
            await this.SelectStoreUsage(SelectStoreUsageEnum.UsePharmacyMainStore, SelectStoreUsageEnum.UseUserResources);
        }
    }

   public DailyDrugOrderGetByDate_Click(){}
    // *****Method declarations end *****

    public initViewModel(): void {
        this._TTObject = new DailyDrugSchedule();
        this.baseDailyDrugScheduleFormViewModel = new BaseDailyDrugScheduleFormViewModel();
        this._ViewModel = this.baseDailyDrugScheduleFormViewModel;
        this.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule = this._TTObject as DailyDrugSchedule;
        this.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule.DailyDrugPatients = new Array<DailyDrugPatient>();
        this.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule.DailyDrugSchOrders = new Array<DailyDrugSchOrder>();
        this.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule.DailyDrugUnDrugs = new Array<DailyDrugUnDrug>();
        this.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule.Store = new Store();
        this.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule.DestinationStore = new Store();
    }

    protected loadViewModel() {
        let that = this;

        that.baseDailyDrugScheduleFormViewModel = this._ViewModel as BaseDailyDrugScheduleFormViewModel;
        that._TTObject = this.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule;
        if (this.baseDailyDrugScheduleFormViewModel == null)
            this.baseDailyDrugScheduleFormViewModel = new BaseDailyDrugScheduleFormViewModel();
        if (this.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule == null)
            this.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule = new DailyDrugSchedule();
        that.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule.DailyDrugPatients = that.baseDailyDrugScheduleFormViewModel.DailyDrugPatientsGridGridList;
        for (let detailItem of that.baseDailyDrugScheduleFormViewModel.DailyDrugPatientsGridGridList) {
            let episodeObjectID = detailItem["Episode"];
            if (episodeObjectID != null && (typeof episodeObjectID === 'string')) {
                let episode = that.baseDailyDrugScheduleFormViewModel.Episodes.find(o => o.ObjectID.toString() === episodeObjectID.toString());
                 if (episode) {
                ...
                if (episode != null) {
                    ...
                    if (patientObjectID != null && (typeof patientObjectID === 'string')) {
                        let patient = that.baseDailyDrugScheduleFormViewModel.Patients.find(o => o.ObjectID.toString() === patientObjectID.toString());
                         if (patient) {
        ...
        that.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule.DailyDrugSchOrders = that.baseDailyDrugScheduleFormViewModel.DailyDrugSchOrdersGridGridList;
        for (let detailItem of that.baseDailyDrugScheduleFormViewModel.DailyDrugSchOrdersGridGridList) {
            let materialObjectID = detailItem["Material"];
            if (materialObjectID != null && (typeof materialObjectID === 'string')) {
                let material = that.baseDailyDrugScheduleFormViewModel.Materials.find(o => o.ObjectID.toString() === materialObjectID.toString());
                 if (material) {
            ...
            if (dailyDrugPatientObjectID != null && (typeof dailyDrugPatientObjectID === 'string')) {
                let dailyDrugPatient = that.baseDailyDrugScheduleFormViewModel.DailyDrugPatients.find(o => o.ObjectID.toString() === dailyDrugPatientObjectID.toString());
                 if (dailyDrugPatient) {
        ...
        that.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule.DailyDrugUnDrugs = that.baseDailyDrugScheduleFormViewModel.DailyDrugUnDrugsGridGridList;
        for (let detailItem of that.baseDailyDrugScheduleFormViewModel.DailyDrugUnDrugsGridGridList) {
            let materialObjectID = detailItem["Material"];
            if (materialObjectID != null && (typeof materialObjectID === 'string')) {
                let material = that.baseDailyDrugScheduleFormViewModel.Materials.find(o => o.ObjectID.toString() === materialObjectID.toString());
                 if (material) {
        ...
        let storeObjectID = that.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule["Store"];
        if (storeObjectID != null && (typeof storeObjectID === 'string')) {
            let store = that.baseDailyDrugScheduleFormViewModel.Stores.find(o => o.ObjectID.toString() === storeObjectID.toString());
             if (store) {
                that.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule.Store = store;
            }
        }
        let destinationStoreObjectID = that.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule["DestinationStore"];
        if (destinationStoreObjectID != null && (typeof destinationStoreObjectID === 'string')) {
            let destinationStore = that.baseDailyDrugScheduleFormViewModel.Stores.find(o => o.ObjectID.toString() === destinationStoreObjectID.toString());
             if (destinationStore) {
                that.baseDailyDrugScheduleFormViewModel._DailyDrugSchedule.DestinationStore = destinationStore;
            }
        }
    }
    ...
    async ngOnInit()  {
        let that = this;
        await this.load(BaseDailyDrugScheduleFormViewModel);
    ...
    public initFormControls(): void {
        ...
        this.patientFullName.Width = 250;
