...
namespace Core.Controllers
{
    ...
    {
        [HttpGet]
        public KScheduleApprovalFormViewModel KScheduleApprovalForm(Guid? id)
        {
            var input = new FormLoadInput();
            input.Id = id;
            return KScheduleApprovalFormLoadInternal(input);
        }

        [HttpPost]
        public KScheduleApprovalFormViewModel KScheduleApprovalFormLoad(FormLoadInput input)
        {
            return KScheduleApprovalFormLoadInternal(input);
        }

        private KScheduleApprovalFormViewModel KScheduleApprovalFormLoadInternal(FormLoadInput input)
        {
            Guid? id = input.Id;
            var formDefID = Guid.Parse("6dd1b8b5-4f91-4062-9481-2314d098f9bf");
            var objectDefID = Guid.Parse("98befb68-75cc-4d99-b76b-55ed1721e254");
            var viewModel = new KScheduleApprovalFormViewModel();
            ...
            if (id.HasValue && id.Value != Guid.Empty)
            {
                using (var objectContext = new TTObjectContext(false))
                {
                    ...
                    PreScript_KScheduleApprovalForm(viewModel, viewModel._KSchedule, objectContext);
        ...
        [HttpPost]
        public BaseViewModel KScheduleApprovalForm(KScheduleApprovalFormViewModel viewModel)
        {
            var retViewModel = new BaseViewModel();
            var formDefID = Guid.Parse("6dd1b8b5-4f91-4062-9481-2314d098f9bf");
            using (var objectContext = new TTObjectContext(false))
            {
                PreMatch_KScheduleApprovalForm(viewModel, viewModel._KSchedule.ObjectID);
                ...
                if (viewModel.StockActionOutDetailsGridList != null)
                {
                    foreach (var item in viewModel.StockActionOutDetailsGridList)
                    {
                        ...
                        if (kScheduleMaterialsImported.UserSelectedOutableTrx.HasValue && kScheduleMaterialsImported.UserSelectedOutableTrx.Value)
                        {
                            foreach (OuttableLotDTO outtable in viewModel.OuttableLotList.Where(x => x.StockActionDetailOrderNo == kScheduleMaterialsImported.ChattelDocDetailOrderNo))
                            {
                                ...
                                outtableLot.StockActionDetailOut = kScheduleMaterialsImported;
                ...
                PostScript_KScheduleApprovalForm(viewModel, kSchedule, transDef, objectContext);
                ...
                AfterContextSaveScript_KScheduleApprovalForm(viewModel, kSchedule, transDef, objectContext);
        ...
        void ContextToViewModel(KScheduleApprovalFormViewModel viewModel, TTObjectContext objectContext)
        {
...
{
    public partial class KScheduleApprovalFormViewModel : BaseViewModel
    {
