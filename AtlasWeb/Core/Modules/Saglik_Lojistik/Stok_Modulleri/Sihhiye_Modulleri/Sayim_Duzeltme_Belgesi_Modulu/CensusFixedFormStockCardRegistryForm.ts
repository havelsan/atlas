...
import { CensusFixedFormStockCardRegistryFormViewModel } from './CensusFixedFormStockCardRegistryFormViewModel';
...
@Component({
    selector: 'CensusFixedFormStockCardRegistryForm',
    templateUrl: './CensusFixedFormStockCardRegistryForm.html',
    providers: [MessageService]
})
export class CensusFixedFormStockCardRegistryForm extends BaseCensusFixedForm implements OnInit {
    public StockActionInDetailsColumns = [];
    public StockActionOutDetailsColumns = [];
    public StockActionSignDetailsColumns = [];
    public censusFixedFormStockCardRegistryFormViewModel: CensusFixedFormStockCardRegistryFormViewModel = new CensusFixedFormStockCardRegistryFormViewModel();
    public get _CensusFixed(): CensusFixed {
        return this._TTObject as CensusFixed;
    }
    private CensusFixedFormStockCardRegistryForm_DocumentUrl: string = '/api/CensusFixedService/CensusFixedFormStockCardRegistryForm';
    constructor(protected httpService: NeHttpService,
                protected messageService: MessageService,
                protected ngZone: NgZone) {
        super(httpService, messageService, ngZone);
        this._DocumentServiceUrl = this.CensusFixedFormStockCardRegistryForm_DocumentUrl;
        this.initViewModel();
    ...
    protected async PreScript() {
        super.PreScript();
        if (this.StockActionInDetails.Rows) {
            let countIn: number = 1;
            for (let row of this.StockActionInDetails.Rows) {
                (<StockActionDetail>row.TTObject).ChattelDocDetailOrderNo = countIn;
                countIn++;
            }
        }
        if (this.StockActionOutDetails.Rows) {
            let countOut: number = 1;
            for (let row of this.StockActionOutDetails.Rows) {
    ...
    public initViewModel(): void {
        ...
        this.censusFixedFormStockCardRegistryFormViewModel = new CensusFixedFormStockCardRegistryFormViewModel();
        this._ViewModel = this.censusFixedFormStockCardRegistryFormViewModel;
        this.censusFixedFormStockCardRegistryFormViewModel._CensusFixed = this._TTObject as CensusFixed;
        this.censusFixedFormStockCardRegistryFormViewModel._CensusFixed.BudgetTypeDefinition = new BudgetTypeDefinition();
        this.censusFixedFormStockCardRegistryFormViewModel._CensusFixed.Store = new Store();
        this.censusFixedFormStockCardRegistryFormViewModel._CensusFixed.CensusFixedInMaterials = new Array<CensusFixedMaterialIn>();
        this.censusFixedFormStockCardRegistryFormViewModel._CensusFixed.StockActionSignDetails = new Array<StockActionSignDetail>();
        this.censusFixedFormStockCardRegistryFormViewModel._CensusFixed.CensusFixedOutMaterials = new Array<CensusFixedMaterialOut>();
    }

    protected loadViewModel() {
        let that = this;

        that.censusFixedFormStockCardRegistryFormViewModel = this._ViewModel as CensusFixedFormStockCardRegistryFormViewModel;
        that._TTObject = this.censusFixedFormStockCardRegistryFormViewModel._CensusFixed;
        if (this.censusFixedFormStockCardRegistryFormViewModel == null)
            this.censusFixedFormStockCardRegistryFormViewModel = new CensusFixedFormStockCardRegistryFormViewModel();
        if (this.censusFixedFormStockCardRegistryFormViewModel._CensusFixed == null)
            this.censusFixedFormStockCardRegistryFormViewModel._CensusFixed = new CensusFixed();
        let budgetTypeDefinitionObjectID = that.censusFixedFormStockCardRegistryFormViewModel._CensusFixed["BudgetTypeDefinition"];
        if (budgetTypeDefinitionObjectID != null && (typeof budgetTypeDefinitionObjectID === 'string')) {
            let budgetTypeDefinition = that.censusFixedFormStockCardRegistryFormViewModel.BudgetTypeDefinitions.find(o => o.ObjectID.toString() === budgetTypeDefinitionObjectID.toString());
             if (budgetTypeDefinition) {
                that.censusFixedFormStockCardRegistryFormViewModel._CensusFixed.BudgetTypeDefinition = budgetTypeDefinition;
            }
        }
        let storeObjectID = that.censusFixedFormStockCardRegistryFormViewModel._CensusFixed["Store"];
        if (storeObjectID != null && (typeof storeObjectID === 'string')) {
            let store = that.censusFixedFormStockCardRegistryFormViewModel.Stores.find(o => o.ObjectID.toString() === storeObjectID.toString());
             if (store) {
                that.censusFixedFormStockCardRegistryFormViewModel._CensusFixed.Store = store;
            }
        }
        that.censusFixedFormStockCardRegistryFormViewModel._CensusFixed.CensusFixedInMaterials = that.censusFixedFormStockCardRegistryFormViewModel.StockActionInDetailsGridList;
        for (let detailItem of that.censusFixedFormStockCardRegistryFormViewModel.StockActionInDetailsGridList) {
            let materialObjectID = detailItem["Material"];
            if (materialObjectID != null && (typeof materialObjectID === 'string')) {
                let material = that.censusFixedFormStockCardRegistryFormViewModel.Materials.find(o => o.ObjectID.toString() === materialObjectID.toString());
                 if (material) {
                ...
                if (material != null) {
                    ...
                    if (stockCardObjectID != null && (typeof stockCardObjectID === 'string')) {
                        let stockCard = that.censusFixedFormStockCardRegistryFormViewModel.StockCards.find(o => o.ObjectID.toString() === stockCardObjectID.toString());
                         if (stockCard) {
                        ...
                        if (stockCard != null) {
                            ...
                            if (distributionTypeObjectID != null && (typeof distributionTypeObjectID === 'string')) {
                                let distributionType = that.censusFixedFormStockCardRegistryFormViewModel.DistributionTypeDefinitions.find(o => o.ObjectID.toString() === distributionTypeObjectID.toString());
                                 if (distributionType) {
            ...
            if (stockLevelTypeObjectID != null && (typeof stockLevelTypeObjectID === 'string')) {
                let stockLevelType = that.censusFixedFormStockCardRegistryFormViewModel.StockLevelTypes.find(o => o.ObjectID.toString() === stockLevelTypeObjectID.toString());
                 if (stockLevelType) {
                    detailItem.StockLevelType = stockLevelType;
                }
            }
        }
        that.censusFixedFormStockCardRegistryFormViewModel._CensusFixed.StockActionSignDetails = that.censusFixedFormStockCardRegistryFormViewModel.StockActionSignDetailsGridList;
        for (let detailItem of that.censusFixedFormStockCardRegistryFormViewModel.StockActionSignDetailsGridList) {
            let signUserObjectID = detailItem["SignUser"];
            if (signUserObjectID != null && (typeof signUserObjectID === 'string')) {
                let signUser = that.censusFixedFormStockCardRegistryFormViewModel.ResUsers.find(o => o.ObjectID.toString() === signUserObjectID.toString());
                 if (signUser) {
                    detailItem.SignUser = signUser;
                }
            }
        }
        that.censusFixedFormStockCardRegistryFormViewModel._CensusFixed.CensusFixedOutMaterials = that.censusFixedFormStockCardRegistryFormViewModel.StockActionOutDetailsGridList;
        for (let detailItem of that.censusFixedFormStockCardRegistryFormViewModel.StockActionOutDetailsGridList) {
            let materialObjectID = detailItem["Material"];
            if (materialObjectID != null && (typeof materialObjectID === 'string')) {
                let material = that.censusFixedFormStockCardRegistryFormViewModel.Materials.find(o => o.ObjectID.toString() === materialObjectID.toString());
                 if (material) {
    ...
    async ngOnInit()  {
        let that = this;
        await this.load(CensusFixedFormStockCardRegistryFormViewModel);
        ...
        this.FormCaption = i18n("M21414", "Sayım Düzeltme Belgesi ( Stok Kart Kayıt )");
