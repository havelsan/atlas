...
{
    [HttpGet]
    public CensusFixedApprovalFormViewModel CensusFixedApprovalForm(Guid? id)
    {
        var input = new FormLoadInput();
        input.Id = id;
        return CensusFixedApprovalFormLoadInternal(input);
    }

    [HttpPost]
    public CensusFixedApprovalFormViewModel CensusFixedApprovalFormLoad(FormLoadInput input)
    {
        return CensusFixedApprovalFormLoadInternal(input);
    }

    private CensusFixedApprovalFormViewModel CensusFixedApprovalFormLoadInternal(FormLoadInput input)
    {
        Guid? id = input.Id;
        var formDefID = Guid.Parse("f1a58c60-b8f7-4e12-854f-c8249359eae3");
        var objectDefID = Guid.Parse("6fa99efd-e615-41a6-ae85-3eaa77d730d6");
        var viewModel = new CensusFixedApprovalFormViewModel();
        ...
        if (id.HasValue && id.Value != Guid.Empty)
        {
            using (var objectContext = new TTObjectContext(false))
            {
                ...
                PreScript_CensusFixedApprovalForm(viewModel, viewModel._CensusFixed, objectContext);
    ...
    [HttpPost]
    public BaseViewModel CensusFixedApprovalForm(CensusFixedApprovalFormViewModel viewModel)
    {
        ...
        using (var objectContext = new TTObjectContext(false))
        {
            ...
            PostScript_CensusFixedApprovalForm(viewModel, censusFixed, transDef, objectContext);
            ...
            AfterContextSaveScript_CensusFixedApprovalForm(viewModel, censusFixed, transDef, objectContext);
    ...
    partial void PreScript_CensusFixedApprovalForm(CensusFixedApprovalFormViewModel viewModel, CensusFixed censusFixed, TTObjectContext objectContext);
    partial void PostScript_CensusFixedApprovalForm(CensusFixedApprovalFormViewModel viewModel, CensusFixed censusFixed, TTDefinitionManagement.TTObjectStateTransitionDef transDef, TTObjectContext objectContext);
    ...
    void ContextToViewModel(CensusFixedApprovalFormViewModel viewModel, TTObjectContext objectContext)
    {
...
{
    public partial class CensusFixedApprovalFormViewModel : BaseViewModel
    {
