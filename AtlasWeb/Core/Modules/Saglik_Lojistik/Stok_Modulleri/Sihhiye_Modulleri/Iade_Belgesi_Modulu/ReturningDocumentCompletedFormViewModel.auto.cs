...
{
    [HttpGet]
    public ReturningDocumentCompletedFormViewModel ReturningDocumentCompletedForm(Guid? id)
    {
        var input = new FormLoadInput();
        input.Id = id;
        return ReturningDocumentCompletedFormLoadInternal(input);
    }

    [HttpPost]
    public ReturningDocumentCompletedFormViewModel ReturningDocumentCompletedFormLoad(FormLoadInput input)
    {
        return ReturningDocumentCompletedFormLoadInternal(input);
    }

    private ReturningDocumentCompletedFormViewModel ReturningDocumentCompletedFormLoadInternal(FormLoadInput input)
    {
        Guid? id = input.Id;
        var formDefID = Guid.Parse("682ebf3d-0f52-4990-b674-b1569562d980");
        var objectDefID = Guid.Parse("ca3f49f3-f0ed-449c-a31a-32534effc1d0");
        var viewModel = new ReturningDocumentCompletedFormViewModel();
        ...
        if (id.HasValue && id.Value != Guid.Empty)
        {
            using (var objectContext = new TTObjectContext(false))
            {
                ...
                PreScript_ReturningDocumentCompletedForm(viewModel, viewModel._ReturningDocument, objectContext);
    ...
    [HttpPost]
    public BaseViewModel ReturningDocumentCompletedForm(ReturningDocumentCompletedFormViewModel viewModel)
    {
        ...
        using (var objectContext = new TTObjectContext(false))
        {
            ...
            if (viewModel.DocumentRecordLogsGridList != null)
            {
                foreach (var item in viewModel.DocumentRecordLogsGridList)
                {
                    ...
                    if (((ITTObject)documentRecordLogsImported).IsDeleted)
                        continue;
                    documentRecordLogsImported.StockAction = returningDocument;
                }
            }
            ...
            PostScript_ReturningDocumentCompletedForm(viewModel, returningDocument, transDef, objectContext);
            ...
            AfterContextSaveScript_ReturningDocumentCompletedForm(viewModel, returningDocument, transDef, objectContext);
    ...
    partial void PreScript_ReturningDocumentCompletedForm(ReturningDocumentCompletedFormViewModel viewModel, ReturningDocument returningDocument, TTObjectContext objectContext);
    partial void PostScript_ReturningDocumentCompletedForm(ReturningDocumentCompletedFormViewModel viewModel, ReturningDocument returningDocument, TTDefinitionManagement.TTObjectStateTransitionDef transDef, TTObjectContext objectContext);
    ...
    void ContextToViewModel(ReturningDocumentCompletedFormViewModel viewModel, TTObjectContext objectContext)
    {
        viewModel.DocumentRecordLogsGridList = viewModel._ReturningDocument.DocumentRecordLogs.OfType<DocumentRecordLog>().ToArray();
...
{
    public partial class ReturningDocumentCompletedFormViewModel : BaseViewModel
    {
