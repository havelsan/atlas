...
import { BaseMainStoreStockTransferFormViewModel } from './BaseMainStoreStockTransferFormViewModel';
...
@Component({
    selector: 'BaseMainStoreStockTransferForm',
    templateUrl: './BaseMainStoreStockTransferForm.html',
    providers: [MessageService]
})
export class BaseMainStoreStockTransferForm extends StockActionBaseForm implements OnInit {
    ...
    MainStoreStockTransferMaterials: TTVisual.ITTGrid;
    MaterialBarcode: TTVisual.ITTTextBoxColumn;
    MaterialMainStoreStockTransferMat: TTVisual.ITTListBoxColumn;
    ...
    public baseMainStoreStockTransferFormViewModel: BaseMainStoreStockTransferFormViewModel = new BaseMainStoreStockTransferFormViewModel();
    public get _MainStoreStockTransfer(): MainStoreStockTransfer {
        return this._TTObject as MainStoreStockTransfer;
    }
    private BaseMainStoreStockTransferForm_DocumentUrl: string = '/api/MainStoreStockTransferService/BaseMainStoreStockTransferForm';
    constructor(protected httpService: NeHttpService,
                protected messageService: MessageService,
                protected ngZone: NgZone) {
        super(httpService, messageService, ngZone);
        this._DocumentServiceUrl = this.BaseMainStoreStockTransferForm_DocumentUrl;
        this.initViewModel();
    ...
    public onMaterialGridRowUpdated(event: any) {
        //console.log(event);
        let data = <MainStoreStockTransferMat>event.key;

        if (data.RequestAmount != null) {
            (<MainStoreStockTransferMat>data).Amount = data.RequestAmount;
        }

        if (data.RequestAmount > data.StoreStock) {
            data.RequestAmount = data.StoreStock;
        }
    ...
    private async MainStoreStockTransferMaterials_CellContentClick(rowIndex: number, columnIndex: number): Promise<void> {
        if (this.MainStoreStockTransferMaterials.CurrentCell.OwningColumn.Name === 'Detail') {
            this.ShowStockActionDetailForm(<StockActionDetail>this.MainStoreStockTransferMaterials.CurrentCell.OwningRow.TTObject);
        }
    }
    private async MainStoreStockTransferMaterials_CellDoubleClick(rowIndex: number, columnIndex: number): Promise<void> {
    }
    ...
    public async TTTeslimAlanButon_Click(): Promise<void> {
        ...
        if (String.isNullOrEmpty(key)) {
            ...
            this._MainStoreStockTransfer.MKYS_TeslimAlanObjID = selectedPersonel.ObjectID;
        }
    }
    public async TTTeslimEdenButon_Click(): Promise<void> {
        ...
        if (String.isNullOrEmpty(key)) {
            ...
            this._MainStoreStockTransfer.MKYS_TeslimEdenObjID = selectedPersonel.ObjectID;
        }
    }
    protected async PreScript() {
        super.PreScript();
        if (this._MainStoreStockTransfer.Store != null) {
            this.MaterialMainStoreStockTransferMat.ListFilterExpression = 'STOCKS.STORE= \'' + this._MainStoreStockTransfer.Store.ObjectID + '\' AND STOCKS.INHELD > 0';
        }
    }

    public async MainStoreStockTransferMaterials_CellValueChanged(data: any, rowIndex: number, columnIndex: number): Promise<void> {
        let mainStoreStockTransferMat: MainStoreStockTransferMat = <MainStoreStockTransferMat>(data.Row.TTObject);
        if (mainStoreStockTransferMat.Status === undefined) {
            mainStoreStockTransferMat.Status = StockActionDetailStatusEnum.New;
            let stockLeveltypeArray: Array<StockLevelType> = await StockLevelTypeService.GetStockLevelType(StockLevelTypeEnum.New);
            mainStoreStockTransferMat.StockLevelType = stockLeveltypeArray[0];
        }
        if (data.Column.Name === 'MaterialMainStoreStockTransferMat' || data.Column.Name === 'StockLevelType') {

            if (mainStoreStockTransferMat.Material != null && mainStoreStockTransferMat.StockLevelType != null) {
                if (mainStoreStockTransferMat.Material.ObjectID != null) {

                    let stockInheld: number =
                        await StockLevelService.StockInheldWithStockLevelByBudgetType(mainStoreStockTransferMat.Material.ObjectID, this._MainStoreStockTransfer.Store.ObjectID,
                            mainStoreStockTransferMat.StockLevelType.ObjectID, this._MainStoreStockTransfer.DestinationStore.ObjectID);
                    mainStoreStockTransferMat.StoreStock = stockInheld;

                }
            }
        }
        if (data.Column.Name === 'RequestAmountSubStoreStockTransferMat') {
            if (mainStoreStockTransferMat.RequestAmount != null) {
                (<MainStoreStockTransferMat>data.Row.TTObject).Amount = mainStoreStockTransferMat.RequestAmount;
            }
            if (mainStoreStockTransferMat.RequestAmount > mainStoreStockTransferMat.StoreStock) {
                mainStoreStockTransferMat.RequestAmount = mainStoreStockTransferMat.StoreStock;
            }
        }
    }
    ...
    public initViewModel(): void {
        ...
        this.baseMainStoreStockTransferFormViewModel = new BaseMainStoreStockTransferFormViewModel();
        this._ViewModel = this.baseMainStoreStockTransferFormViewModel;
        this.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer = this._TTObject as MainStoreStockTransfer;
        this.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer.Store = new Store();
        this.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer.DestinationStore = new Store();
        this.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer.StockActionSignDetails = new Array<StockActionSignDetail>();
        this.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer.MainStoreStockTransferMats = new Array<MainStoreStockTransferMat>();
    }

    protected loadViewModel() {
        let that = this;

        that.baseMainStoreStockTransferFormViewModel = this._ViewModel as BaseMainStoreStockTransferFormViewModel;
        that._TTObject = this.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer;
        if (this.baseMainStoreStockTransferFormViewModel == null) {
            this.baseMainStoreStockTransferFormViewModel = new BaseMainStoreStockTransferFormViewModel();
        }
        if (this.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer == null) {
            this.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer = new MainStoreStockTransfer();
        }
        let storeObjectID = that.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer['Store'];
        if (storeObjectID != null && (typeof storeObjectID === 'string')) {
            let store = that.baseMainStoreStockTransferFormViewModel.Stores.find(o => o.ObjectID.toString() === storeObjectID.toString());
             if (store) {
                that.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer.Store = store;
            }
        }
        let destinationStoreObjectID = that.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer['DestinationStore'];
        if (destinationStoreObjectID != null && (typeof destinationStoreObjectID === 'string')) {
            let destinationStore = that.baseMainStoreStockTransferFormViewModel.Stores.find(o => o.ObjectID.toString() === destinationStoreObjectID.toString());
             if (destinationStore) {
                that.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer.DestinationStore = destinationStore;
            }
        }
        that.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer.StockActionSignDetails = that.baseMainStoreStockTransferFormViewModel.StockActionSignDetailsGridList;
        for (let detailItem of that.baseMainStoreStockTransferFormViewModel.StockActionSignDetailsGridList) {
            let signUserObjectID = detailItem['SignUser'];
            if (signUserObjectID != null && (typeof signUserObjectID === 'string')) {
                let signUser = that.baseMainStoreStockTransferFormViewModel.ResUsers.find(o => o.ObjectID.toString() === signUserObjectID.toString());
                 if (signUser) {
                    detailItem.SignUser = signUser;
                }
            }
        }
        that.baseMainStoreStockTransferFormViewModel._MainStoreStockTransfer.MainStoreStockTransferMats = that.baseMainStoreStockTransferFormViewModel.MainStoreStockTransferMaterialsGridList;
        for (let detailItem of that.baseMainStoreStockTransferFormViewModel.MainStoreStockTransferMaterialsGridList) {
            let materialObjectID = detailItem['Material'];
            if (materialObjectID != null && (typeof materialObjectID === 'string')) {
                let material = that.baseMainStoreStockTransferFormViewModel.Materials.find(o => o.ObjectID.toString() === materialObjectID.toString());
                 if (material) {
                ...
                if (material != null) {
                    ...
                    if (stockCardObjectID != null && (typeof stockCardObjectID === 'string')) {
                        let stockCard = that.baseMainStoreStockTransferFormViewModel.StockCards.find(o => o.ObjectID.toString() === stockCardObjectID.toString());
                         if (stockCard) {
                        ...
                        if (stockCard != null) {
                            ...
                            if (distributionTypeObjectID != null && (typeof distributionTypeObjectID === 'string')) {
                                let distributionType =
                                    that.baseMainStoreStockTransferFormViewModel.DistributionTypeDefinitions.find(o => o.ObjectID.toString() === distributionTypeObjectID.toString());
                                 if (distributionType) {
            ...
            if (stockLevelTypeObjectID != null && (typeof stockLevelTypeObjectID === 'string')) {
                let stockLevelType = that.baseMainStoreStockTransferFormViewModel.StockLevelTypes.find(o => o.ObjectID.toString() === stockLevelTypeObjectID.toString());
                 if (stockLevelType) {
    ...
    async ngOnInit()  {
        let that = this;
        await this.load(BaseMainStoreStockTransferFormViewModel);
    ...
    public async btnAddClick() {
        if (this.selectedMaterial == null)
            ...
            if (!this._MainStoreStockTransfer.MainStoreStockTransferMats.find(x => x.Material.ObjectID == this.selectedMaterial.ObjectID)) {
                let newRow: MainStoreStockTransferMat = new MainStoreStockTransferMat();
                ...
                if (this._MainStoreStockTransfer.TransactionDate != null)
                    newRow.VatRate = await MaterialService.GetVatrateFromMaterialTS(this.selectedMaterial, this._MainStoreStockTransfer.TransactionDate);

                if (this.selectedStockLevelType != null) {
                    let stockInheld: number = await StockLevelService.StockInheldWithStockLevel(this.selectedMaterial.ObjectID, this._MainStoreStockTransfer.Store.ObjectID, this.selectedStockLevelType.ObjectID);
                    newRow.StoreStock = stockInheld;
                }

                this._MainStoreStockTransfer.MainStoreStockTransferMats.push(newRow);
            }
            else
    ...
    public MainStoreStockTransferMaterialsColumns = [
    ...
    public async stateChange(event: FormSaveInfo) {
        this.gridMainStoreStockTransferMats.instance.closeEditCell();
        this.gridMainStoreStockTransferMats.instance.saveEditData();
    ...
    @ViewChild('gridMainStoreStockTransferMats') gridMainStoreStockTransferMats: DxDataGridComponent;
    async GridMainStoreStockTransferMats_CellContentClicked(data: any) {
        let that = this;
                ...
                if (data.row != null) {
                    if (data.row.key != null) {
                        if (data.row.key.IsNew) {
                            this.gridMainStoreStockTransferMats.instance.deleteRow(data.rowIndex);
                        }
                        else {
                            data.key.EntityState = EntityStateEnum.Deleted;
                            this.gridMainStoreStockTransferMats.instance.filter(['EntityState', '<>', 1]);
                            this.gridMainStoreStockTransferMats.instance.refresh();
    ...
    public initFormControls(): void {
        ...
        this.MaterialMainStoreStockTransferMat.AutoCompleteDialogWidth = '50%';
        ...
        this.Controls = [this.Description, this.ID, this.labelMKYS_EMalzemeGrup, this.MKYS_EMalzemeGrup, this.labelStore, this.Store, this.labelDestinationStore,
        this.DestinationStore, this.labelActionDate, this.ActionDate, this.labelID, this.DescriptionAndSignTabControl, this.SignTabpage, this.StockActionSignDetails,
        this.SignUserType, this.SignUser, this.IsDeputy, this.DescriptionTabpage, this.tttabcontrol1, this.MaterialTabPage, this.MainStoreStockTransferMaterials, this.MKYS_TeslimAlan, this.MKYS_TeslimEden, this.ttlabel1, this.labelMKYS_TeslimEden, this.labelMKYS_TeslimAlan, this.TTTeslimAlanButon, this.TTTeslimEdenButon];
    }
}