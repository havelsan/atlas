...
import { GrantMaterialCancelFormViewModel } from './GrantMaterialCancelFormViewModel';
...
@Component({
    selector: 'GrantMaterialCancelForm',
    templateUrl: './GrantMaterialCancelForm.html',
    providers: [MessageService]
})
export class GrantMaterialCancelForm extends BaseGrantMaterialForm implements OnInit {
    ...
    public grantMaterialCancelFormViewModel: GrantMaterialCancelFormViewModel = new GrantMaterialCancelFormViewModel();
    public get _GrantMaterial(): GrantMaterial {
        return this._TTObject as GrantMaterial;
    }
    private GrantMaterialCancelForm_DocumentUrl: string = '/api/GrantMaterialService/GrantMaterialCancelForm';
    constructor(protected httpService: NeHttpService,
        protected messageService: MessageService,
        protected ngZone: NgZone) {
        super(httpService, messageService, ngZone);
        this._DocumentServiceUrl = this.GrantMaterialCancelForm_DocumentUrl;
        this.initViewModel();
    ...
    protected async ClientSidePreScript(): Promise<void> {
        ...
        if (this._GrantMaterial.CurrentStateDefID.toString() == GrantMaterial.GrantMaterialStates.Completed.id.toString()) {
    ...
    public async reGenerateAction_Click(): Promise<void> {
        ...
        try {
            result = await StockActionService.ReGenerateMyGrantMaterial(this._GrantMaterial.ObjectID.toString());
    ...
    public async SendToMKYS_Click(): Promise<void> {
            ...
            try {
                ...
                for (let log of this._GrantMaterial.DocumentRecordLogs) {
                    if (log.ReceiptNumber != null) {
                        this.mkysSonucMesaj = await StockActionService.SendDeleteMessageToMkysTS(this._GrantMaterial, false, log.ReceiptNumber.Value, params.password);
                    }
                }
        ...
        /*for (let log of this._GrantMaterial.DocumentRecordLogs) {
            //TODO: İlayda (MKYS kullanıcı adı şifre)
            if (log.ReceiptNumber != null) {
                this.mkysSonucMesaj = await StockActionService.SendDeleteMessageToMkysTS(this._GrantMaterial, false, log.ReceiptNumber.Value,"*");
            }
        }
        this.popupVisible = true;*/
    }
    ...
    onSelectionChange(andata: any){}
    // *****Method declarations end *****

    public initViewModel(): void {
        this._TTObject = new GrantMaterial();
        this.grantMaterialCancelFormViewModel = new GrantMaterialCancelFormViewModel();
        this._ViewModel = this.grantMaterialCancelFormViewModel;
        this.grantMaterialCancelFormViewModel._GrantMaterial = this._TTObject as GrantMaterial;
        this.grantMaterialCancelFormViewModel._GrantMaterial.DocumentRecordLogs = new Array<DocumentRecordLog>();
        this.grantMaterialCancelFormViewModel._GrantMaterial.BudgetTypeDefinition = new BudgetTypeDefinition();
        this.grantMaterialCancelFormViewModel._GrantMaterial.Store = new Store();
        this.grantMaterialCancelFormViewModel._GrantMaterial.GrantMaterialDetails = new Array<GrantMaterialDetail>();
        this.grantMaterialCancelFormViewModel._GrantMaterial.StockActionSignDetails = new Array<StockActionSignDetail>();
    }
    ...
    protected loadViewModel() {
        ...
        that.grantMaterialCancelFormViewModel = this._ViewModel as GrantMaterialCancelFormViewModel;
        that._TTObject = this.grantMaterialCancelFormViewModel._GrantMaterial;
        if (this.grantMaterialCancelFormViewModel == null) {
            this.grantMaterialCancelFormViewModel = new GrantMaterialCancelFormViewModel();
        }
        if (this.grantMaterialCancelFormViewModel._GrantMaterial == null) {
            this.grantMaterialCancelFormViewModel._GrantMaterial = new GrantMaterial();
        }
        that.grantMaterialCancelFormViewModel._GrantMaterial.DocumentRecordLogs = that.grantMaterialCancelFormViewModel.DocumentRecordLogsGridList;
        for (let detailItem of that.grantMaterialCancelFormViewModel.DocumentRecordLogsGridList) {
        }
        let budgetTypeDefinitionObjectID = that.grantMaterialCancelFormViewModel._GrantMaterial['BudgetTypeDefinition'];
        if (budgetTypeDefinitionObjectID != null && (typeof budgetTypeDefinitionObjectID === 'string')) {
            let budgetTypeDefinition = that.grantMaterialCancelFormViewModel.BudgetTypeDefinitions.find(o => o.ObjectID.toString() === budgetTypeDefinitionObjectID.toString());
             if (budgetTypeDefinition) {
                that.grantMaterialCancelFormViewModel._GrantMaterial.BudgetTypeDefinition = budgetTypeDefinition;
            }
        }
        let storeObjectID = that.grantMaterialCancelFormViewModel._GrantMaterial['Store'];
        if (storeObjectID != null && (typeof storeObjectID === 'string')) {
            let store = that.grantMaterialCancelFormViewModel.Stores.find(o => o.ObjectID.toString() === storeObjectID.toString());
             if (store) {
                that.grantMaterialCancelFormViewModel._GrantMaterial.Store = store;
            }
        }
        that.grantMaterialCancelFormViewModel._GrantMaterial.GrantMaterialDetails = that.grantMaterialCancelFormViewModel.GrantMaterialDetailsGridList;
        for (let detailItem of that.grantMaterialCancelFormViewModel.GrantMaterialDetailsGridList) {
            let materialObjectID = detailItem['Material'];
            if (materialObjectID != null && (typeof materialObjectID === 'string')) {
                let material = that.grantMaterialCancelFormViewModel.Materials.find(o => o.ObjectID.toString() === materialObjectID.toString());
                 if (material) {
                ...
                if (material != null) {
                    ...
                    if (stockCardObjectID != null && (typeof stockCardObjectID === 'string')) {
                        let stockCard = that.grantMaterialCancelFormViewModel.StockCards.find(o => o.ObjectID.toString() === stockCardObjectID.toString());
                         if (stockCard) {
                        ...
                        if (stockCard != null) {
                            ...
                            if (distributionTypeObjectID != null && (typeof distributionTypeObjectID === 'string')) {
                                let distributionType = that.grantMaterialCancelFormViewModel.DistributionTypeDefinitions.find(o => o.ObjectID.toString() === distributionTypeObjectID.toString());
                                 if (distributionType) {
            ...
            if (stockLevelTypeObjectID != null && (typeof stockLevelTypeObjectID === 'string')) {
                let stockLevelType = that.grantMaterialCancelFormViewModel.StockLevelTypes.find(o => o.ObjectID.toString() === stockLevelTypeObjectID.toString());
                 if (stockLevelType) {
                    detailItem.StockLevelType = stockLevelType;
                }
            }
        }
        that.grantMaterialCancelFormViewModel._GrantMaterial.StockActionSignDetails = that.grantMaterialCancelFormViewModel.StockActionSignDetailsGridList;
        for (let detailItem of that.grantMaterialCancelFormViewModel.StockActionSignDetailsGridList) {
            let signUserObjectID = detailItem['SignUser'];
            if (signUserObjectID != null && (typeof signUserObjectID === 'string')) {
                let signUser = that.grantMaterialCancelFormViewModel.ResUsers.find(o => o.ObjectID.toString() === signUserObjectID.toString());
                 if (signUser) {
    ...
    async ngOnInit() {
        let that = this;
        await this.load(GrantMaterialCancelFormViewModel);
        ...
        this.FormCaption = i18n("M11407", "Bağış / Yardım ( İptal Edildi )");
    ...
    public initFormControls(): void {
        ...
        this.DocumentTransactionTypeDocumentRecordLog, this.BUDGETYPE, this.SubjectDocumentRecordLog, this.ReceiptNumber,this.DocumentRecordLogDeleteMKYS , this.DescriptionsDocumentRecordLog];
        this.GrantMaterialDetailsColumns = [this.MaterialGrantMaterialDetail, this.Barcode, this.DistiributionType, this.AmountGrantMaterialDetail,
        ...
        this.RetrievalYearGrantMaterialDetail, this.ProductionDateGrantMaterialDetail];
        ...
        this.NotDiscountedUnitPrice, this.UnitPrice, this.LotNoGrantMaterialDetail, this.ExpirationDateGrantMaterialDetail, this.StockLevelType,
        this.RetrievalYearGrantMaterialDetail, this.ProductionDateGrantMaterialDetail, this.MKYS_ETedarikTuru,this.DocumentRecordLogDeleteMKYS , this.labelMKYS_ETedarikTuru];
