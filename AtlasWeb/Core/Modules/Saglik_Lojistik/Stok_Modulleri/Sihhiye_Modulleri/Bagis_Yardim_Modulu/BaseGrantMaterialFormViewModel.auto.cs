...
{
    [HttpGet]
    public BaseGrantMaterialFormViewModel BaseGrantMaterialForm(Guid? id)
    {
        var input = new FormLoadInput();
        input.Id = id;
        return BaseGrantMaterialFormLoadInternal(input);
    }

    [HttpPost]
    public BaseGrantMaterialFormViewModel BaseGrantMaterialFormLoad(FormLoadInput input)
    {
        return BaseGrantMaterialFormLoadInternal(input);
    }

    private BaseGrantMaterialFormViewModel BaseGrantMaterialFormLoadInternal(FormLoadInput input)
    {
        Guid? id = input.Id;
        var formDefID = Guid.Parse("9858ca16-f3af-4fa9-b0a8-60e02da37838");
        var objectDefID = Guid.Parse("6dfc9fd8-2488-4d88-a109-c205546f8c25");
        var viewModel = new BaseGrantMaterialFormViewModel();
        ...
        if (id.HasValue && id.Value != Guid.Empty)
        {
            using (var objectContext = new TTObjectContext(false))
            {
                ...
                PreScript_BaseGrantMaterialForm(viewModel, viewModel._GrantMaterial, objectContext);
    ...
    [HttpPost]
    public BaseViewModel BaseGrantMaterialForm(BaseGrantMaterialFormViewModel viewModel)
    {
        ...
        using (var objectContext = new TTObjectContext(false))
        {
            ...
            PostScript_BaseGrantMaterialForm(viewModel, grantMaterial, transDef, objectContext);
            ...
            AfterContextSaveScript_BaseGrantMaterialForm(viewModel, grantMaterial, transDef, objectContext);
    ...
    partial void PreScript_BaseGrantMaterialForm(BaseGrantMaterialFormViewModel viewModel, GrantMaterial grantMaterial, TTObjectContext objectContext);
    partial void PostScript_BaseGrantMaterialForm(BaseGrantMaterialFormViewModel viewModel, GrantMaterial grantMaterial, TTDefinitionManagement.TTObjectStateTransitionDef transDef, TTObjectContext objectContext);
    partial void AfterContextSaveScript_BaseGrantMaterialForm(BaseGrantMaterialFormViewModel viewModel, GrantMaterial grantMaterial, TTDefinitionManagement.TTObjectStateTransitionDef transDef, TTObjectContext objectContext);
    void ContextToViewModel(BaseGrantMaterialFormViewModel viewModel, TTObjectContext objectContext)
    {
...
{
    public partial class BaseGrantMaterialFormViewModel : BaseViewModel
    {
