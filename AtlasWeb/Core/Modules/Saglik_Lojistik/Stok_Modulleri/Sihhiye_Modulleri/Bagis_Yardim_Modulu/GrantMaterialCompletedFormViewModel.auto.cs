...
{
    [HttpGet]
    public GrantMaterialCompletedFormViewModel GrantMaterialCompletedForm(Guid? id)
    {
        var input = new FormLoadInput();
        input.Id = id;
        return GrantMaterialCompletedFormLoadInternal(input);
    }

    [HttpPost]
    public GrantMaterialCompletedFormViewModel GrantMaterialCompletedFormLoad(FormLoadInput input)
    {
        return GrantMaterialCompletedFormLoadInternal(input);
    }

    private GrantMaterialCompletedFormViewModel GrantMaterialCompletedFormLoadInternal(FormLoadInput input)
    {
        Guid? id = input.Id;
        var formDefID = Guid.Parse("9d6b76b5-9795-4216-b64b-40b5f6b5c52c");
        var objectDefID = Guid.Parse("6dfc9fd8-2488-4d88-a109-c205546f8c25");
        var viewModel = new GrantMaterialCompletedFormViewModel();
        ...
        if (id.HasValue && id.Value != Guid.Empty)
        {
            using (var objectContext = new TTObjectContext(false))
            {
                ...
                PreScript_GrantMaterialCompletedForm(viewModel, viewModel._GrantMaterial, objectContext);
    ...
    [HttpPost]
    public BaseViewModel GrantMaterialCompletedForm(GrantMaterialCompletedFormViewModel viewModel)
    {
        ...
        using (var objectContext = new TTObjectContext(false))
        {
            ...
            if (viewModel.DocumentRecordLogsGridList != null)
            {
                foreach (var item in viewModel.DocumentRecordLogsGridList)
                {
                    ...
                    if (((ITTObject)documentRecordLogsImported).IsDeleted)
                        continue;
                    documentRecordLogsImported.StockAction = grantMaterial;
                }
            }
            ...
            PostScript_GrantMaterialCompletedForm(viewModel, grantMaterial, transDef, objectContext);
            ...
            AfterContextSaveScript_GrantMaterialCompletedForm(viewModel, grantMaterial, transDef, objectContext);
    ...
    partial void PreScript_GrantMaterialCompletedForm(GrantMaterialCompletedFormViewModel viewModel, GrantMaterial grantMaterial, TTObjectContext objectContext);
    partial void PostScript_GrantMaterialCompletedForm(GrantMaterialCompletedFormViewModel viewModel, GrantMaterial grantMaterial, TTDefinitionManagement.TTObjectStateTransitionDef transDef, TTObjectContext objectContext);
    ...
    void ContextToViewModel(GrantMaterialCompletedFormViewModel viewModel, TTObjectContext objectContext)
    {
        viewModel.DocumentRecordLogsGridList = viewModel._GrantMaterial.DocumentRecordLogs.OfType<DocumentRecordLog>().ToArray();
...
{
    public partial class GrantMaterialCompletedFormViewModel : BaseViewModel
    {
