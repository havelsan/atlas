...
import { CostActionComplatedFormViewModel } from './CostActionComplatedFormViewModel';
...
@Component({
    selector: 'CostActionComplatedForm',
    templateUrl: './CostActionComplatedForm.html',
    providers: [MessageService]
})
export class CostActionComplatedForm extends BaseCostActionForm implements OnInit {
    public CostActionMaterialsColumns = [];
    public costActionComplatedFormViewModel: CostActionComplatedFormViewModel = new CostActionComplatedFormViewModel();
    public get _CostAction(): CostAction {
        return this._TTObject as CostAction;
    }
    private CostActionComplatedForm_DocumentUrl: string = '/api/CostActionService/CostActionComplatedForm';
    constructor(protected httpService: NeHttpService,
                protected messageService: MessageService,
                protected ngZone: NgZone) {
        super(httpService, messageService, ngZone);
        this._DocumentServiceUrl = this.CostActionComplatedForm_DocumentUrl;
        this.initViewModel();
    ...
    public initViewModel(): void {
        ...
        this.costActionComplatedFormViewModel = new CostActionComplatedFormViewModel();
        this._ViewModel = this.costActionComplatedFormViewModel;
        this.costActionComplatedFormViewModel._CostAction = this._TTObject as CostAction;
        this.costActionComplatedFormViewModel._CostAction.CostActionMaterials = new Array<CostActionMaterial>();
        this.costActionComplatedFormViewModel._CostAction.Store = new Store();
    }

    protected loadViewModel() {
        let that = this;

        that.costActionComplatedFormViewModel = this._ViewModel as CostActionComplatedFormViewModel;
        that._TTObject = this.costActionComplatedFormViewModel._CostAction;
        if (this.costActionComplatedFormViewModel == null)
            this.costActionComplatedFormViewModel = new CostActionComplatedFormViewModel();
        if (this.costActionComplatedFormViewModel._CostAction == null)
            this.costActionComplatedFormViewModel._CostAction = new CostAction();
        that.costActionComplatedFormViewModel._CostAction.CostActionMaterials = that.costActionComplatedFormViewModel.CostActionMaterialsGridList;
        for (let detailItem of that.costActionComplatedFormViewModel.CostActionMaterialsGridList) {
            let materialObjectID = detailItem["Material"];
            if (materialObjectID != null && (typeof materialObjectID === 'string')) {
                let material = that.costActionComplatedFormViewModel.Materials.find(o => o.ObjectID.toString() === materialObjectID.toString());
                 if (material) {
                    detailItem.Material = material;
                }
            }
        }
        let storeObjectID = that.costActionComplatedFormViewModel._CostAction["Store"];
        if (storeObjectID != null && (typeof storeObjectID === 'string')) {
            let store = that.costActionComplatedFormViewModel.Stores.find(o => o.ObjectID.toString() === storeObjectID.toString());
             if (store) {
                that.costActionComplatedFormViewModel._CostAction.Store = store;
            }
        }
    }
    ...
    async ngOnInit()  {
        let that = this;
        await this.load(CostActionComplatedFormViewModel);
        this.FormCaption = "Aylık Ortalama Maliyet Analizi ( Tamamlandı )";
