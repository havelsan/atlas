...
import { CostActionApprovalFormViewModel } from './CostActionApprovalFormViewModel';
...
@Component({
    selector: 'CostActionApprovalForm',
    templateUrl: './CostActionApprovalForm.html',
    providers: [MessageService]
})
export class CostActionApprovalForm extends BaseCostActionForm implements OnInit {
    public CostActionMaterialsColumns = [];
    public costActionApprovalFormViewModel: CostActionApprovalFormViewModel = new CostActionApprovalFormViewModel();
    public get _CostAction(): CostAction {
        return this._TTObject as CostAction;
    }
    private CostActionApprovalForm_DocumentUrl: string = '/api/CostActionService/CostActionApprovalForm';
    constructor(protected httpService: NeHttpService,
                protected messageService: MessageService,
                protected ngZone: NgZone) {
        super(httpService, messageService, ngZone);
        this._DocumentServiceUrl = this.CostActionApprovalForm_DocumentUrl;
        this.initViewModel();
    ...
    public initViewModel(): void {
        ...
        this.costActionApprovalFormViewModel = new CostActionApprovalFormViewModel();
        this._ViewModel = this.costActionApprovalFormViewModel;
        this.costActionApprovalFormViewModel._CostAction = this._TTObject as CostAction;
        this.costActionApprovalFormViewModel._CostAction.CostActionMaterials = new Array<CostActionMaterial>();
        this.costActionApprovalFormViewModel._CostAction.Store = new Store();
    }

    protected loadViewModel() {
        let that = this;

        that.costActionApprovalFormViewModel = this._ViewModel as CostActionApprovalFormViewModel;
        that._TTObject = this.costActionApprovalFormViewModel._CostAction;
        if (this.costActionApprovalFormViewModel == null)
            this.costActionApprovalFormViewModel = new CostActionApprovalFormViewModel();
        if (this.costActionApprovalFormViewModel._CostAction == null)
            this.costActionApprovalFormViewModel._CostAction = new CostAction();
        that.costActionApprovalFormViewModel._CostAction.CostActionMaterials = that.costActionApprovalFormViewModel.CostActionMaterialsGridList;
        for (let detailItem of that.costActionApprovalFormViewModel.CostActionMaterialsGridList) {
            let materialObjectID = detailItem["Material"];
            if (materialObjectID != null && (typeof materialObjectID === 'string')) {
                let material = that.costActionApprovalFormViewModel.Materials.find(o => o.ObjectID.toString() === materialObjectID.toString());
                 if (material) {
                    detailItem.Material = material;
                }
            }
        }
        let storeObjectID = that.costActionApprovalFormViewModel._CostAction["Store"];
        if (storeObjectID != null && (typeof storeObjectID === 'string')) {
            let store = that.costActionApprovalFormViewModel.Stores.find(o => o.ObjectID.toString() === storeObjectID.toString());
             if (store) {
                that.costActionApprovalFormViewModel._CostAction.Store = store;
            }
        }
    }
    ...
    async ngOnInit()  {
        let that = this;
        await this.load(CostActionApprovalFormViewModel);
        this.FormCaption = "Aylık Ortalama Maliyet Analizi ( Onay )";
