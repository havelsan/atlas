 ...
 * Based on: http://xxxxxx.com/wiki/Uniform_Resource_Identifier
 */
...
export class Uri implements IUri, IEquatable<IUri>
{
	...
	{
		let u = (!uri || Type.isString(uri))
			? Uri.parse(<string>uri) : <IUri>uri;

		let scheme: SchemeValue = u.scheme || defaults && defaults.scheme || "http";
	...
	{
		let result: IUri = null;
		let ex = tryParse(url, (out) => {result = out; });
	 ...
	 * http://xxxxxx.com/en-us/library/system.uri.segments%28v=vs.110%29.aspx
	 *
	 * <h5><b>Example:</b></h5>
	...
	{
		let convertedUri: Uri = uri as Uri;
		if ( convertedUri != null)
			return convertedUri.absoluteUri;
...
function uriToString(uri: IUri): string
{
	...
	let scheme       = getScheme(uri.scheme),
...
function tryParse(url: string, out: (result: IUri) => void): Exception
{
	...
	if (i != -1)
	{
		let scheme = trim(url.substring(0, i)), c = /:$/;
		if (!c.test(scheme))
	...
	if (i != -1)
	{
		let port = parseInt(trim(url.substring(i + 1)));
		if (isNaN(port))
