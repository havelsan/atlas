	...
	decodeValues: boolean = true): void
{
	...
	{
		let entries = query.split(ENTRY_SEPARATOR);
		for (let entry of entries)
		{
			 ...
			 */
			let si = entry.indexOf(KEY_VALUE_SEPARATOR);
			if (si != -1)
			{
...
{
	let result: IMap<Primitive|Primitive[]> = {};
	parse(query,
		(key, value) =>
		{
			if ((key) in (result))
...
{
	let result: IStringKeyValuePair<Primitive>[] = [];
	parse(query,
