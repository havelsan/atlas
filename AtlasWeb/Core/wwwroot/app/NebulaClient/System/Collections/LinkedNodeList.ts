...
implements ILinkedNodeList<TNode>, IEnumerateEach<TNode>, IDisposable
{
	...
	get count(): number
	{
		...
		let next = this._first, i: number = 0;
		while (next)
		{
			i++;
			next = next.next;
	...
	clear(): number
	{
		let _ = this, n: TNode, cF: number = 0, cL: number = 0;
	...
	indexOf(node: TNode): number
	{
		if (node && (node.previous || node.next))
		{
			...
			let index = 0;
			let c: TNode, n: TNode = this._first;
			do {
				c = n;
	...
	removeNode(node: TNode): boolean
	{
		...
		const _ = this;
		let prev = node.previous, next = node.next, a: boolean = false, b: boolean = false;
