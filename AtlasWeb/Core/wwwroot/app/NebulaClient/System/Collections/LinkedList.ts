 ...
 * http://xxxxxx.com/simulating-a-queue
 *
 * Adding to an array is very fast, but modifying is slow.
 * LinkedList wins when modifying contents.
 * http://xxxxxx.com/questions/166884/array-versus-linked-list
 *****************************/
...
function ensureExternal<T>(node: InternalNode<T>, list: LinkedList<T>): ILinkedListNode<T>
{
	...
	let external: ILinkedListNode<T> = node.external;
	if (!external)
...
extends CollectionBase<T> implements ILinkedList<T>
{
	...
	get firstValue(): T
	{
		let li = this._listInternal, node = li && li.first;
	...
	get lastValue(): T
	{
		let li = this._listInternal, node = li && li.last;
		return node ? node.value : VOID0;
	}
	...
	{
		let li = this._listInternal, node = li && li.getNodeAt(index);
		return node ? node.value : VOID0;
	}
