...
extends CollectionBase<IKeyValuePair<TKey, TValue>> implements IDictionary<TKey, TValue>
{
	...
	public push(item: KeyValuePair<TKey, TValue>): boolean {
		return this._addInternal(item);
	}
	...
	addByKeyValue(key: TKey, value: TValue): boolean
	{
		...
		{
			let ex = new InvalidOperationException("Adding a key/value when the key already exists.");
			ex.data['key'] = key;
			ex.data['value'] = value;
			throw ex;
		}
	...
	setValue(key: TKey, value: TValue): boolean
	{
		...
		let changed = false, old = _.getValue(key); // get the old value here and pass to internal.
	...
	{
		let e = this.getEnumerator(), equal: (a: any, b: any, strict?: boolean) => boolean = areEqual;
	...
	{
		let _ = this, count = 0, equal: (a: any, b: any, strict?: boolean) => boolean = areEqual;
	...
	{
		const _ = this;
		let ver: number, keys: TKey[], len: number, i = 0;
		return new EnumeratorBase<IKeyValuePair<TKey, TValue>>(
			...
			(yielder) =>
			{
				...
				while (i < len)
				{
					let key = keys[i++], value = _.getValue(key);
