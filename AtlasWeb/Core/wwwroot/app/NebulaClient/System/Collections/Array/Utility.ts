	...
	length: number = Infinity): TDestination
{
	...
	let maxLength = source.length - sourceIndex;
	...
	let newLength = destinationIndex + length;
	if (newLength > destination.length) (<any>destination).length = newLength;

	for (let i = 0; i < length; i++)
	{
		(<any>destination)[destinationIndex + i] = source[sourceIndex + i];
	}
	...
	equalityComparer: EqualityComparison<T> = areEqual): boolean
{
	...
	let ok = !len || !contains(array, item, equalityComparer);
	...
	step: number = 1): number[]
{
	...
	let result = initialize<number>(count);
	for (let i = 0; i < count; i++)
	{
		result[i] = first;
		first += step;
