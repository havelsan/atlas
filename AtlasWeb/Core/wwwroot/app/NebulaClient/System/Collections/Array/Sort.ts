...
{
	let nanHasEquivalent = !Type.isTrueNaN(equivalentToNaN);

	return (a: TSource|TSource[], b: TSource|TSource[]): CompareResult =>
	{
		// Use an array always to ensure a single code path.
		let aValue = ensureArray(selector(a));
		let bValue = ensureArray(selector(b));
		let len = Math.min(aValue.length, bValue.length);

		let oArray: Order[] = Array.isArray(order) ? order : null;
		for (let i = 0; i < len; i++)
		{
