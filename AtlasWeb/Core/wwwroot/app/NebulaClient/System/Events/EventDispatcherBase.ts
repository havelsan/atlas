...
{
	let p: IEntryParams = this.params;
...
class EventDispatcherBase extends DisposableBase implements IEventDispatcher
{
	...
	dispatchEvent(e: any, params?: any): boolean
	{
		...
		if (typeof e == "string")
		{
			event = <IEventBase<any>>(Event && Object.create(Event) || {});
			if (!params)
				params = {};
		...
		let entries: EventDispatcherEntry<IEntryParams>[] = l.filter(e => e.type == type); //, propagate = true, prevent = false;
		if (!entries.length)
			return false;
		...
		entries.forEach(
			entry =>
			{
				let newEvent = Object.create(Event.prototype);
				shallowCopy(event, newEvent);
				(<any>newEvent).target = this;
