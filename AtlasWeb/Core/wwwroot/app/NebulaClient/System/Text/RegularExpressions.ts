 ...
 * Named groups based on: http://xxxxxx.com/2011/08/02/javascript-regexp-match-named-captures/
...
/**
 * http://xxxxxx.com/en-us/library/system.text.regularexpressions.regexoptions%28v=vs.110%29.aspx
 */
...
export class Regex
{
		...
		...extra: RegexOptions.Literal[])
	{
		...
		let ignoreWhiteSpace = flags.indexOf(_W) != -1;
	...
	match(input: string, startIndex: number = 0): Match
	{
		...
		let m = new Match(r[0], first, groups, groupMap);
		m.freeze();
		return m;
	}

	matches(input: string): ReadonlyArray<Match>
	{
		let matches: Match[] = [], m: Match, p = 0, end = input && input.length || 0;
		...
		count: number = Infinity): string
	{
		...
		let result: string[] = [];
		let p = 0, end = input.length, isEvaluator = typeof r == "function";
