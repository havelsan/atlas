import { IComparable } from "../../System/IComparable";
import { IConvertible } from "../../System/IConvertable";
import { IEquatable } from "../../System/IEquatable";
import { OverflowException } from "../../System/Exceptions/OverflowException";
import { ArgumentException } from "../../System/Exceptions/ArgumentException";
import { NotImplementedException } from "../../System/Exceptions/NotImplementedException";
import { IFormatProvider } from "../../System/IFormatProvider";

export class BigCurrency implements IComparable<any>, IConvertible, IEquatable<number>
{
    public static Length: number = 34;
    public static Precision: number = 6;
    private static _minValue: number = -9999999999999999999999999999.999999;
    private static _maxValue: number = 9999999999999999999999999999.999999;
    public static get MinValue(): BigCurrency {
        return BigCurrency.constructFromNumber(BigCurrency._minValue);
    }
    public static get MaxValue(): BigCurrency {
        return BigCurrency.constructFromNumber(BigCurrency._maxValue);
    }
    private m_value: number;
    private CheckRange(): void {
        if (this.m_value < BigCurrency._minValue || this.m_value > BigCurrency._maxValue)
            throw new OverflowException("Value was either too large or too small for a Currency.");
    }

    private static constructFromNumber(d: number) {
        let currency = new BigCurrency();
        currency.m_value = <number>d;
        currency.CheckRange();
        return currency;
    }

    constructor() {
    }

    public compareTo(value: any): number {
        if (value === null) {
            return 1;
        }
        if (!(typeof value === "number") && !(value instanceof BigCurrency)) {
            throw new ArgumentException("Value must be decimal or Currency.");
        }
        let d: number;
        if (typeof value === "number")
            d = <number>value;
        else d = (<BigCurrency>value).m_value;
        if (this.m_value < d) {
            return -1;
        }
        if (this.m_value > d) {
            return 1;
        }
        return 0;
    }

    public equals(obj: number): boolean {
        //TODO: Implement equals
        throw new NotImplementedException();
    }

    public getHashCode(): number {
        //TODO: Implement getHashCode
        throw new NotImplementedException();
    }

    toBoolean(provider: IFormatProvider): boolean {
        //TODO: Implement boolean conversion
        throw new NotImplementedException();
    }

    toNumber(provider: IFormatProvider): number {
        //TODO: Implement number conversion
        return this.m_value;
    }

    toString(provider: IFormatProvider): string {
        //TODO: Implement string conversion
        return this.m_value.toString();
    }
}
