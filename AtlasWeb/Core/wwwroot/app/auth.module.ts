...
export function tokenGetterFromSessionStorage() {
  return sessionStorage.getItem(AuthConfigDefaults.tokenName) as string;
}
...
const AuthConfigDefaults: IAuthConfig = {
  ...
  tokenGetter: tokenGetterFromSessionStorage,
  noJwtError: false,
  noClientCheck: false,
  globalHeaders: [],
  noTokenScheme: false
...
export class AuthHttp {
  ...
  public request(url: string | Request, options?: RequestOptionsArgs): Observable<Response> {
    ...
    return defer(() => {
      ...
      if (!token) {
        token = AuthConfigDefaults.tokenGetter();
      }
      if (token instanceof Promise) {
        return from(token).pipe(mergeMap((jwtToken: string) => this.requestWithToken(req, jwtToken)));
      } else {
...
export class JwtHelper {
  ...
  // credits for decoder goes to http://xxxxxx.com/atk
  ...
  // http://xxxxxx.com/en/docs/Web/API/WindowBase64/Base64_encoding_and_decoding#The_Unicode_Problem
...
export const authConfigDefault = new AuthConfig({
  tokenName: 'token',
...
export class AtlasAuthModule {
	static dynamicComponentsMap = {

	};

  constructor( @Optional() @SkipSelf() parentModule: AtlasAuthModule) {
    if (parentModule) {
      throw new Error(
  ...
  static forRoot(): ModuleWithProviders<AtlasAuthModule> {
    return {
      ngModule: AtlasAuthModule,
      providers: [
        { provide: AuthConfig, useValue: authConfigDefault }
      ]
    };
  }

  static forRootWithConfig(config: AuthConfig): ModuleWithProviders<AtlasAuthModule> {
    return {
      ngModule: AtlasAuthModule,
      providers: [
