...
namespace TTFormClasses
{
    ...
    public partial class PresReturningDocumentApprovalForm : BasePresReturningDocumentForm
    {
        ...
        {
#region PresReturningDocumentApprovalForm_PreScript
    base.PreScript();
            foreach (PresReturningDocMaterial presReturningDocMaterial in _PresReturningDocument.ReturningDocumentMaterials)
            {

                if (presReturningDocMaterial.Amount == null || presReturningDocMaterial.Amount == 0 || presReturningDocMaterial.Amount != presReturningDocMaterial.RequireAmount)
                    presReturningDocMaterial.Amount = presReturningDocMaterial.RequireAmount;
            }
            if (_PresReturningDocument.CurrentStateDefID == PresReturningDocument.States.Approval)
            {
                if (_PresReturningDocument.ReturnDepStoreObjectID != null)
                {
                    this.DropStateButton(PresReturningDocument.States.New);
                    this.DropStateButton(PresReturningDocument.States.Cancelled);
                }
            }

            if (_PresReturningDocument.StockActionSignDetails.Count == 0)
            {
                StockActionSignDetail stockActionSignDetail = _PresReturningDocument.StockActionSignDetails.AddNew();
