...
namespace TTReportClasses
{
    ...
    /// </summary>
    public partial class HospitalCompHEKGraphRep : TTReport
    {
        ...
        {
            public HospitalCompHEKGraphRep MyParentReport
            {
                get { return (HospitalCompHEKGraphRep)ParentReport; }
            }
            ...
            public TTReportField CurrentUser11 { get {return Body().CurrentUser11;} }
            ...
            public partial class MAINGroupBody : TTReportSection
            {
                ...
                public MAINGroupBody(TTReportGroup parentGroup) : base(parentGroup)
                {
                    ...
                    PrintDate11 = ReportObjects.AddNewField(MyParentReport.SetFieldDefaultProperties(), 20, 200, 45, 205, false);
                    ...
                    PrintDate11.TextFont.Name = "Arial";
                    PrintDate11.TextFont.Size = 8;
                    PrintDate11.TextFont.CharSet = 162;
                    PrintDate11.Value = @"{@printdate@}";

                    CurrentUser11 = ReportObjects.AddNewField(MyParentReport.SetFieldDefaultProperties(), 146, 200, 173, 205, false);
                    ...
                    CurrentUser11.TextFont.Name = "Arial";
                    ...
                    PageNumber11 = ReportObjects.AddNewField(MyParentReport.SetFieldDefaultProperties(), 255, 200, 280, 205, false);
                    ...
                    NewLine1111111 = ReportObjects.AddNewShape(MyParentReport.SetShapeDefaultProperties(), ShapeTypeEnum.shtLine, 20, 200, 280, 200, false);
...
#region MAIN BODY_Script
            ...
            // Hastana için yapılan bu.

            
            //IBindingList hekList = Repair.GetHEKListFromRepairQuery(this.MyParentReport.RuntimeParameters.YEAR.Value);
            //IBindingList normalList = Repair.GetNormalListFromRepairQuery(this.MyParentReport.RuntimeParameters.YEAR.Value);


            //BindingList<Repair.GetHEKListFromRepairQuery_Class> hekList = Repair.GetHEKListFromRepairQuery(this.MyParentReport.RuntimeParameters.YEAR.Value);
            //BindingList<Repair.GetNormalListFromRepairQuery_Class> normalList = Repair.GetNormalListFromRepairQuery(this.MyParentReport.RuntimeParameters.YEAR.Value);
            //mca
            ...
            IBindingList allList = Repair.GetHEKListQuery(ctx,(int)(this.MyParentReport.RuntimeParameters.YEAR.Value));

            foreach (Repair rp1 in allList)
            {
            ...
            for(int i= 0;i< 12 ;i++)
            {
                if(i!=11)
                {
                    if((monthTA[i]>monthTA[(i+1)])&&(monthTA[i]>Buyuk))
                    {
                        Buyuk = monthTA[i];
                    }
                    else if((monthTA[11]>monthTA[0])&&(monthTA[11]>Buyuk))
                    {
                        Buyuk = monthTA[11];
                    }
                    
                    
                    if((monthHEK[i]>monthHEK[(i+1)])&&(monthHEK[i]>Buyuk2))
                    {
                        Buyuk2 = monthHEK[i];
                    }
                    else if((monthHEK[11]>monthHEK[0])&&(monthHEK[11]>Buyuk2))
                    {
                        Buyuk2 = monthHEK[11];
                    }
                }
            ...
            if(EnBuyuk >= 200)
            {
                deger=( EnBuyuk - EnBuyuk%200 )+200;
                deger2= (deger / 10);
            }
            else
            {
                deger=( EnBuyuk - EnBuyuk%20 )+20;
                deger2= (deger / 10);
            }

            

            GridLegend100.CalcValue = deger.ToString(); //     (10 * GraphScale).ToString();
            GridLegend90.CalcValue =  (Convert.ToInt32(GridLegend100.CalcValue)- deger2).ToString();                 // (9 * GraphScale).ToString();
            GridLegend80.CalcValue =(Convert.ToInt32(GridLegend90.CalcValue)- deger2).ToString();  // (8 * GraphScale).ToString();
            GridLegend70.CalcValue =(Convert.ToInt32(GridLegend80.CalcValue)- deger2).ToString();  // (7 * GraphScale).ToString();
            GridLegend60.CalcValue = (Convert.ToInt32(GridLegend70.CalcValue)- deger2).ToString(); //(6 * GraphScale).ToString();
            GridLegend50.CalcValue = (Convert.ToInt32(GridLegend60.CalcValue)- deger2).ToString(); //(5 * GraphScale).ToString();
            GridLegend40.CalcValue = (Convert.ToInt32(GridLegend50.CalcValue)- deger2).ToString(); //(4 * GraphScale).ToString();
            GridLegend30.CalcValue =(Convert.ToInt32(GridLegend40.CalcValue)- deger2).ToString();  //(3 * GraphScale).ToString();
            GridLegend20.CalcValue =(Convert.ToInt32(GridLegend30.CalcValue)- deger2).ToString();  //(2 * GraphScale).ToString();
            GridLegend10.CalcValue =(Convert.ToInt32(GridLegend20.CalcValue)- deger2).ToString();  //(1 * GraphScale).ToString();
            ...
            if (ocakTA != 0)
            {
                hesapla = (100 * (double)ocakHEK / (double)ocakTA);
                OCAKORAN.CalcValue = "%" +  hesapla.ToString("##.##");
                hesapla = 0.00;
            }
            else if (ocakTA == 0)
            { OCAKORAN.CalcValue = "-"; }
            ...
            if (subatTA != 0)
            {
                hesapla = (100 * (double)subatHEK / (double)subatTA);
                SUBATORAN.CalcValue = "%" + hesapla.ToString("##.##");
                hesapla = 0.00;
            }
            else if (subatTA == 0)
            { SUBATORAN.CalcValue = "-"; }
            ...
            if (martTA != 0)
            {
                hesapla = (100 * (double)martHEK / (double)martTA);
                MARTORAN.CalcValue = "%" + hesapla.ToString("##.##");
                hesapla = 0.00;
            }
            else if (martTA == 0)
            { MARTORAN.CalcValue = "-"; }
            ...
            if (nisanTA != 0)
            {
                NISANORAN.CalcValue = "%" + (100 * (double)nisanHEK / (double)nisanTA).ToString("##.##");
                hesapla = 0.00;
            }
            else if (nisanTA == 0)
            { NISANORAN.CalcValue = "-"; }
            ...
            if (mayisTA != 0)
            {
                hesapla = (100 * (double)mayisHEK / (double)mayisTA);
                MAYISORAN.CalcValue = "%" + hesapla.ToString("##.##");
                hesapla = 0.00;
            }
            else if (mayisTA == 0)
            { MAYISORAN.CalcValue = "-"; }
            ...
            if (haziranTA != 0)
            {
                hesapla =(100 * (double)haziranHEK / (double)haziranTA) ;
                HAZIRANORAN.CalcValue = "%" + hesapla.ToString("##.##");
                hesapla = 0.00;
            }
            else if (haziranTA == 0)
            ...
            if (temmuzTA != 0)
            {
                hesapla =(100 * (double)temmuzHEK / (double)temmuzTA);
                TEMMUZORAN.CalcValue = "%" + hesapla.ToString("##.##");
                hesapla = 0.00;
            }
            else if (temmuzTA == 0)
            ...
            if (agustosTA != 0)
            {
                hesapla =(100 * (double)agustosHEK / (double)agustosTA);
                AGUSTOSORAN.CalcValue = "%" + hesapla.ToString("##.##");
                hesapla = 0.00;
            }
            else if (agustosTA == 0)
            ...
            if (eylulTA != 0)
            {
                hesapla =(100 * (double)eylulHEK / (double)eylulTA);
                EYLULORAN.CalcValue = "%" + hesapla.ToString("##.##");
                hesapla = 0.00;
            }
            else if (eylulTA == 0)
            { EYLULORAN.CalcValue = "-"; }
            ...
            if (ekimTA != 0)
            {
                hesapla = (100 * (double)ekimHEK / (double)ekimTA);
                EKIMORAN.CalcValue = "%" + hesapla.ToString("##.##");
                hesapla = 0.00;
            }
            else if (ekimTA == 0)
            { EKIMORAN.CalcValue = "-"; }
            ...
            if (kasimTA != 0)
            {
                hesapla =(100 * (double)kasimHEK / (double)kasimTA);
                KASIMORAN.CalcValue = "%" + hesapla.ToString("##.##");
                hesapla = 0.00;
            }
            else if (kasimTA == 0)
            { KASIMORAN.CalcValue = "-"; }
            ...
            if (aralikTA != 0)
            {
                hesapla =(100 * (double)aralikHEK / (double)aralikTA);
                ARALIKORAN.CalcValue = "%" + hesapla.ToString("##.##");
                hesapla = 0.00;
            }
            else if (aralikTA == 0)
        ...
        public HospitalCompHEKGraphRep()
        {
