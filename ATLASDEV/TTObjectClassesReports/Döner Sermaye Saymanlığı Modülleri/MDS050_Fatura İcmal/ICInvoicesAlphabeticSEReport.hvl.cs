...
{
    public partial class ICInvoicesAlphabeticSEReport : TTReport
    {
#region Methods
   Currency PageTotal = 0;
        ...
        {
            public ICInvoicesAlphabeticSEReport MyParentReport
            {
                get { return (ICInvoicesAlphabeticSEReport)ParentReport; }
            }
            ...
            public TTReportField SEOBJECTID { get {return Header().SEOBJECTID;} }
            ...
            protected override TTReportNqlDataSet GetReportNqlDataSet()
            {
                ...
                list[0] = new TTReportNqlData<SubEpisodeProtocol.GetByInvoiceCollectionAlphabetic_Class>("GetByInvoiceCollectionAlphabetic", SubEpisodeProtocol.GetByInvoiceCollectionAlphabetic((Guid)TTObjectDefManager.Instance.DataTypes["Guid"].ConvertValue(MyParentReport.RuntimeParameters.TTOBJECTID)));
            ...
            public partial class PARTAGroupHeader : TTReportSection
            {
                ...
                public TTReportField SEOBJECTID; 
                public PARTAGroupHeader(TTReportGroup parentGroup) : base(parentGroup)
                {
                    ...
                    ICDOBJECTID = ReportObjects.AddNewField(MyParentReport.SetFieldDefaultProperties(), 252, 1, 277, 6, false);
                    ...
                    SEOBJECTID = ReportObjects.AddNewField(MyParentReport.SetFieldDefaultProperties(), 280, 1, 305, 6, false);
                    SEOBJECTID.Name = "SEOBJECTID";
                    SEOBJECTID.Visible = EvetHayirEnum.ehHayir;
                    SEOBJECTID.FieldType = ReportFieldTypeEnum.ftVariable;
                    SEOBJECTID.TextFont.Name = "Arial Narrow";
                    SEOBJECTID.Value = @"{#SEOBJECTID#}";
                ...
                {
                    SubEpisodeProtocol.GetByInvoiceCollectionAlphabetic_Class dataset_GetByInvoiceCollectionAlphabetic = ParentGroup.rsGroup.GetCurrentRecord<SubEpisodeProtocol.GetByInvoiceCollectionAlphabetic_Class>(0);
                    PIDOBJECTID.CalcValue = (dataset_GetByInvoiceCollectionAlphabetic != null ? Globals.ToStringCore(dataset_GetByInvoiceCollectionAlphabetic.Pidobjectid) : "");
                    ICDOBJECTID.CalcValue = (dataset_GetByInvoiceCollectionAlphabetic != null ? Globals.ToStringCore(dataset_GetByInvoiceCollectionAlphabetic.Icdobjectid) : "");
                    SEOBJECTID.CalcValue = (dataset_GetByInvoiceCollectionAlphabetic != null ? Globals.ToStringCore(dataset_GetByInvoiceCollectionAlphabetic.Seobjectid) : "");
                    return new TTReportObject[] { PIDOBJECTID,ICDOBJECTID,SEOBJECTID};
                }
...
#region HEAD HEADER_Script
                    PayerInvoiceDocument pid = MyParentReport.ReportObjectContext.GetObject<PayerInvoiceDocument>(new Guid(MyParentReport.PARTA.PIDOBJECTID.CalcValue));
            SubEpisode se = MyParentReport.ReportObjectContext.GetObject<SubEpisode>(new Guid(MyParentReport.PARTA.SEOBJECTID.CalcValue));

            if (pid != null && se != null)
            {
                this.SEVKNO.CalcValue = se.Episode.DocumentNumber.Equals("0") ? string.Empty : se.Episode.DocumentNumber;
                this.SEVKTARIHI.CalcValue = se.Episode.DocumentDate.HasValue ? se.Episode.DocumentDate.Value.ToShortDateString() : string.Empty;
                MyParentReport.HPROTNO = se.ProtocolNo;
                this.HPROTNO.CalcValue = MyParentReport.HPROTNO;
                MyParentReport.PatientName = se.Episode.Patient.FullName;
                this.PATIENTNAME.CalcValue = MyParentReport.PatientName;
                this.OPENINGDATE.CalcValue = se.OpeningDate.HasValue ? se.OpeningDate.ToString() : string.Empty;

                if (se.Episode.Patient.UniqueRefNo.HasValue)
                    this.UNIQUEREFNO.CalcValue = se.Episode.Patient.UniqueRefNo.Value.ToString();

                // Servis Adı ve Doktor
                if (se.PatientStatus == SubEpisodeStatusEnum.Outpatient)
                {
                    if (se.PatientAdmission.AdmissionStatus.Value == AdmissionStatusEnum.SaglikKurulu)
                        this.SERVICENAME.CalcValue = se.Episode.EpisodeActions.FirstOrDefault(x => x.GetType() == typeof(HealthCommittee)).MasterResource.Name;
                    else
                        this.SERVICENAME.CalcValue = se.ResSection.Name;

                    this.DOCTORNAME.CalcValue = se.Episode.GetPatientExaminationDoctor()?.Name;
                }
                else
                {
                    this.INPATIENTDATE.CalcValue = se.OpeningDate.HasValue ? se.OpeningDate.ToString() : string.Empty;
                    this.DISCHARGEDATE.CalcValue = se.ClosingDate.HasValue ? se.ClosingDate.ToString() : string.Empty;
                    this.SERVICENAME.CalcValue = se.ResSection.Name;

                    if (se.InpatientAdmission != null && se.InpatientAdmission.IsCancelled == false)
                    {
                        InPatientTreatmentClinicApplication ipCliniCApp = se.InpatientAdmission.InPatientTreatmentClinicApplications.FirstOrDefault(x => x.IsCancelled == false);
                        if (ipCliniCApp != null)
                            this.DOCTORNAME.CalcValue = ipCliniCApp.ProcedureDoctor.Name;
                    }
                }

                // Tanı
                string diagnosisCodePrimer = string.Empty;
                string diagnosisStrPrimer = string.Empty;
                IList primerDiagnosisList = new List<string>();

                string diagnosisCodeSeconder = string.Empty;
                string diagnosisStrSeconder = string.Empty;
                IList seconderDiagnosisList = new List<string>();
                int seconderDiagnosisExists = 0;

                foreach (DiagnosisGrid dg in se.Diagnosis)
                {
...
#region HEAD FOOTER_Script
            ...
            // Bu altvakaya ait faturalanmış SEP lerin toplam fatura tutarı bulunur, TOTAL buna eşitse
            // (yani o subepisode un son sayfasındaysa) tedavi, ilaç ve sarf toplamları yazılır
            SubEpisode se = MyParentReport.ReportObjectContext.GetObject<SubEpisode>(new Guid(MyParentReport.PARTA.SEOBJECTID.CalcValue));

            if (se != null && !string.IsNullOrWhiteSpace(MyParentReport.PARTA.ICDOBJECTID.CalcValue))
            {
                List<SubEpisodeProtocol> sepList = se.SubEpisodeProtocols.Where(x => x.InvoiceStatus == MedulaSubEpisodeStatusEnum.Invoiced && x.InvoiceCollectionDetail.ObjectID.ToString() == MyParentReport.PARTA.ICDOBJECTID.CalcValue).ToList();

                decimal seInvoiceTotal = 0;
                foreach (SubEpisodeProtocol sep in sepList.Where(x => x.MedulaFaturaTutari.HasValue))
                    seInvoiceTotal += Convert.ToDecimal(sep.MedulaFaturaTutari);

                // SubEpisode un son sayfası ise
                if (this.TOTAL.CalcValue.Equals(seInvoiceTotal.ToString()))
                {
                    decimal drugTotal = 0;
                    decimal materialTotal = 0;

                    // Faturalanmış ilaç ve malzeme Toplamı bulunur
                    foreach (SubEpisodeProtocol sep in sepList)
                    {
                        foreach (AccountTransaction accTrx in sep.AccountTransactions.Select("SUBACTIONMATERIAL IS NOT NULL AND CURRENTSTATEDEFID = '" + AccountTransaction.States.Invoiced + "'"))
                        {
                            if (accTrx.AccountPayableReceivable.Type == APRTypeEnum.PAYER)
                            {
                                if (accTrx.SubActionMaterial.Material is DrugDefinition || accTrx.SubActionMaterial.Material is MagistralPreparationDefinition)
                                    drugTotal += Convert.ToDecimal(accTrx.Amount * accTrx.UnitPrice);
                                else
                                    materialTotal += Convert.ToDecimal(accTrx.Amount * accTrx.UnitPrice);
                            }
                        }
                    }

                    this.TEDAVITOPLAMI.CalcValue = Convert.ToString(seInvoiceTotal - (drugTotal + materialTotal));
                    this.ILACTOPLAMI.CalcValue = drugTotal.ToString();
                    this.SARFTOPLAMI.CalcValue = materialTotal.ToString();
        ...
        public partial class TRXDESCGroup : TTReportGroup
        {
            ...
            {
                TTBaseReportNqlData[] list = new TTBaseReportNqlData[1];
                list[0] = new TTReportNqlData<PayerInvoiceDocument.PayerInvoiceReportQueryBySE_Class>("PayerInvoiceReportQueryBySE", PayerInvoiceDocument.PayerInvoiceReportQueryBySE((Guid)TTObjectDefManager.Instance.DataTypes["Guid"].ConvertValue(MyParentReport.PARTA.PIDOBJECTID.CalcValue),(Guid)TTObjectDefManager.Instance.DataTypes["Guid"].ConvertValue(MyParentReport.PARTA.SEOBJECTID.CalcValue)));
                return new TTReportNqlDataSet(ParentGroup, list);
            }
            ...
            public partial class TRXDESCGroupHeader : TTReportSection
            {
                ...
                override public TTReportObject[] GetCalculatedFields()
                {
                    PayerInvoiceDocument.PayerInvoiceReportQueryBySE_Class dataset_PayerInvoiceReportQueryBySE = ParentGroup.rsGroup.GetCurrentRecord<PayerInvoiceDocument.PayerInvoiceReportQueryBySE_Class>(0);
                    GROUPNAME.CalcValue = (dataset_PayerInvoiceReportQueryBySE != null ? Globals.ToStringCore(dataset_PayerInvoiceReportQueryBySE.Pricinggroupdescription) : "");
        ...
        public partial class MAINGroup : TTReportGroup
        {
            ...
            protected override object[] GetGroupByValues()
            {
                ...
                PayerInvoiceDocument.PayerInvoiceReportQueryBySE_Class dataSet_PayerInvoiceReportQueryBySE = ParentGroup.rsGroup.GetCurrentRecord<PayerInvoiceDocument.PayerInvoiceReportQueryBySE_Class>(0);    
                return new object[] {(dataSet_PayerInvoiceReportQueryBySE==null ? null : dataSet_PayerInvoiceReportQueryBySE.Pricinggroupdescription)};
            }
            private void onConstruct()
            {
                _header = null;
            ...
            public partial class MAINGroupBody : TTReportSection
            {
                ...
                override public TTReportObject[] GetCalculatedFields()
                {
                    PayerInvoiceDocument.PayerInvoiceReportQueryBySE_Class dataset_PayerInvoiceReportQueryBySE = MyParentReport.TRXDESC.rsGroup.GetCurrentRecord<PayerInvoiceDocument.PayerInvoiceReportQueryBySE_Class>(0);
                    ORDERNO.CalcValue = @"";
                    DESCRIPTION.CalcValue = @"";
                    AMOUNT.CalcValue = (dataset_PayerInvoiceReportQueryBySE != null ? Globals.ToStringCore(dataset_PayerInvoiceReportQueryBySE.Amount) : "");
                    UNITPRICE.CalcValue = (dataset_PayerInvoiceReportQueryBySE != null ? Globals.ToStringCore(dataset_PayerInvoiceReportQueryBySE.UnitPrice) : "");
                    ...
                    TRXNAME.CalcValue = (dataset_PayerInvoiceReportQueryBySE != null ? Globals.ToStringCore(dataset_PayerInvoiceReportQueryBySE.Trxname) : "");
                    TRXEXTERNALCODE.CalcValue = (dataset_PayerInvoiceReportQueryBySE != null ? Globals.ToStringCore(dataset_PayerInvoiceReportQueryBySE.Trxexternalcode) : "");
                    PACKAGEOUTREASON.CalcValue = (dataset_PayerInvoiceReportQueryBySE != null ? Globals.ToStringCore(dataset_PayerInvoiceReportQueryBySE.PackageOutReason) : "");
                    PACKAGEOUTREASON.PostFieldValueCalculation();
                    DAY.CalcValue = (dataset_PayerInvoiceReportQueryBySE != null ? Globals.ToStringCore(dataset_PayerInvoiceReportQueryBySE.Day) : "");
                    MONTH.CalcValue = (dataset_PayerInvoiceReportQueryBySE != null ? Globals.ToStringCore(dataset_PayerInvoiceReportQueryBySE.Month) : "");
                    YEAR.CalcValue = (dataset_PayerInvoiceReportQueryBySE != null ? Globals.ToStringCore(dataset_PayerInvoiceReportQueryBySE.Year) : "");
...
#region MAIN_Script
        ...
        public ICInvoicesAlphabeticSEReport()
        {
        ...
        public override void PrepareTTReport(Dictionary<string, object> parameters)
        {
            ...
            Name = "ICINVOICESALPHABETICSEREPORT";
            Caption = "İcmal Faturaları (Kabul Bazlı - Alfabetik)";
